...
export default function SignIn() {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      navigate(
        location.state?.from?.pathname || createPageUrl("AdminDashboard"),
        { replace: true },
      );
    },
    onError: (error) => {
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center px-4 py-12 ${isDark ? "bg-[#0a0d12]" : "bg-slate-50"}`}
    >
      {/* Background Effects */}
      <div className="absolute inset-0 overflow-hidden">
        <div
          className={`absolute top-1/4 left-1/4 w-96 h-96 rounded-full blur-3xl ${isDark ? "bg-amber-500/10" : "bg-amber-500/5"}`}
        ></div>
        <div
          className={`absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full blur-3xl ${isDark ? "bg-blue-500/5" : "bg-blue-500/5"}`}
        ></div>
      </div>
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex items-center gap-3 mb-6">
            ...
            <div className="flex flex-col items-start">
              <span
                className={`font-bold text-xl tracking-tight ${isDark ? "text-white" : "text-slate-900"}`}
              >
                PROP<span className="text-amber-400">CAPITALS</span>
              </span>
              <span
        ...
        {/* Auth Card */}
        <div
          className={`rounded-2xl p-6 sm:p-8 border shadow-2xl ${isDark ? "bg-[#12161d] border-white/10" : "bg-white border-slate-200"}`}
        >
          {/* Admin Panel Badge */}
          <div className="flex items-center justify-center mb-6">
            <div
              className={`flex items-center gap-2 px-4 py-2 rounded-xl ${isDark ? "bg-amber-500/10 border border-amber-500/20" : "bg-amber-50 border border-amber-200"}`}
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label
                className={`text-sm mb-2 block ${isDark ? "text-gray-400" : "text-slate-600"}`}
              >
                {t("signIn.email")}
              </label>
              <div className="relative">
                <Mail
                  className={`absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 ${isDark ? "text-gray-500" : "text-slate-400"}`}
                />
                <input
                  type="email"
                  value={email}
          ...
          {/* Admin Notice */}
          <div
            className={`mt-6 pt-6 border-t ${isDark ? "border-white/10" : "border-slate-200"}`}
          >
            <p
              className={`text-center text-sm ${isDark ? "text-gray-500" : "text-slate-400"}`}
            >
