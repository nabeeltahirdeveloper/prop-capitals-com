...
export default function CRMPipeline() {
  ...
  const fetchLeadsForColumn = useCallback(
    async (status, skip = 0, isRefreshing = false) => {
      try {
        if (!isRefreshing) {
  ...
  const refreshAllColumns = useCallback(async () => {
    setLoading(true);
    const promises = STATUS_COLUMNS.map((col) =>
      fetchLeadsForColumn(col.id, 0, true),
    );
  ...
  const handleDragEnd = async (result) => {
    ...
    const destList =
      sourceStatus === destStatus
        ? sourceList
        : [...leadsByStatus[destStatus].items];
    ...
    try {
      ...
      toast({
        ...
        description: t("crm.pipeline.movedTo", {
          defaultValue: "Lead moved to {{status}}",
          status: t(
            `crm.columns.${destStatus.toLowerCase() === "follow_up" ? "followUp" : destStatus.toLowerCase()}`,
            { defaultValue: destStatus },
          ),
        }),
      });
    } catch (error) {
      ...
      toast({
        ...
        description: t("crm.pipeline.updateError", {
          defaultValue: "Failed to update lead status. Reverting...",
        }),
        variant: "destructive",
      });
  ...
  const handleLeadClick = async (lead) => {
    try {
      ...
      setEditableFields({
        ...
        convertedAt: transformed.convertedAt
          ? new Date(transformed.convertedAt).toISOString().split("T")[0]
          : "",
      });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-foreground">
            {t("crm.pipeline.title", { defaultValue: "Sales Pipeline" })}
          </h1>
          <p className="text-muted-foreground">
            {t("crm.pipeline.subtitle", {
              defaultValue: "Track and manage leads through stages",
            })}
          </p>
        </div>
            ...
            <div className="flex gap-4 h-full min-w-max p-1">
              {STATUS_COLUMNS.map((column) => {
                ...
                return (
                  <div
                    key={column.id}
                    className="w-[320px] flex flex-col gap-3 h-full"
                  >
                    {/* Column Header */}
                    <div
                      className={`p-4 rounded-xl border-t-4 bg-card border border-border shrink-0 ${column.color === "blue" ? "border-t-blue-500" : column.color === "purple" ? "border-t-purple-500" : column.color === "cyan" ? "border-t-cyan-500" : column.color === "yellow" ? "border-t-yellow-500" : column.color === "orange" ? "border-t-orange-500" : column.color === "amber" ? "border-t-amber-500" : "border-t-red-500"}`}
                    >
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          <div
                            className={`p-1.5 rounded-lg ${STATUS_COLORS[column.id]}`}
                          >
                            <column.icon className="w-4 h-4" />
                          </div>
                          <h3 className="font-bold text-foreground text-sm uppercase tracking-wider">
                            {t(
                              `crm.columns.${column.id.toLowerCase() === "follow_up" ? "followUp" : column.id.toLowerCase()}`,
                                ...
                                {(provided, snapshot) => (
                                  ...
                                  >
                                    ...
                                    <div className="flex items-start gap-3">
                                      <div
                                        className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold shrink-0 ${column.color === "blue" ? "bg-blue-600" : column.color === "purple" ? "bg-purple-600" : column.color === "cyan" ? "bg-cyan-600" : column.color === "yellow" ? "bg-yellow-600" : column.color === "orange" ? "bg-orange-600" : column.color === "amber" ? "bg-amber-600" : "bg-red-600"}`}
                                      >
                                        {lead.name?.[0] || "?"}
                                      </div>
                                      <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between gap-2">
                                          ...
                                          <div
                                            className={`w-2 h-2 rounded-full ${lead.onlineStatus === "ONLINE" ? "bg-amber-500" : "bg-muted-foreground/40"}`}
                                          />
                                        </div>
                            ...
                            {currentLeads.length === 0 && !colState.loading && (
                              <div className="flex flex-col items-center justify-center py-8 text-center">
                                <p className="text-muted-foreground text-sm">
                                  {t("crm.pipeline.noLeadsInStage", {
                                    defaultValue: "No leads in this stage",
                                  })}
                                </p>
                              </div>
                            )}
            ...
            <>
              ...
              <div className="p-6 border-b border-border flex items-center justify-between bg-card shrink-0">
                <div className="flex items-center gap-4">
                  <div
                    className={`w-14 h-14 rounded-full flex items-center justify-center text-white font-bold text-xl ${STATUS_COLORS[selectedLead.status] || "bg-slate-600"}`}
                  >
                    {selectedLead.name[0]}
                  </div>
                  <div>
                    <DialogTitle className="text-foreground text-xl font-bold flex items-center gap-2">
                      {selectedLead.name}
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-bold border ${leadOnlineStatusColor(selectedLead.onlineStatus)}`}
                      >
                        {selectedLead.onlineStatus === "ONLINE"
                          ? t("crm.pipeline.online", { defaultValue: "ONLINE" })
                          : t("crm.pipeline.offline", {
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {t("crm.modal.close", { defaultValue: "Close" })}
                  </Button>
                </div>
              </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="relative">
                            <Input
                              ...
                              readOnly
                              className="bg-muted border-border pr-10 text-muted-foreground"
                            />
                            <button
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="relative">
                            <Input
                              type={showSensitive.phone ? "text" : "password"}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="text-xs text-muted-foreground">
                            {t("crm.modal.status", { defaultValue: "Status" })}
                          </label>
                            ...
                            <SelectContent className="bg-card border-border text-foreground">
                              {STATUS_COLUMNS.map((col) => (
                                <SelectItem key={col.id} value={col.id}>
                                  {t(
                                    `crm.columns.${col.id.toLowerCase() === "follow_up" ? "followUp" : col.id.toLowerCase()}`,
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="text-xs text-muted-foreground">
                            {t("crm.modal.age", { defaultValue: "Age" })}
                          </label>
                          <Input
                            type="number"
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 rounded-xl bg-muted border border-border">
                        ...
                        <div className="space-y-1.5">
                          <label className="text-xs text-muted-foreground">
                            {t("crm.modal.convertedDate", {
                              defaultValue: "Converted Date",
                            })}
                          </label>
                          <Input
                            type="date"
                            value={editableFields.convertedAt}
                            readOnly
                            className="bg-card border-border text-muted-foreground"
                          />
                        </div>
                        <div className="col-span-2 space-y-1.5">
                          <label className="text-xs text-muted-foreground">
                            {t("crm.modal.funnelName", {
                              defaultValue: "Funnel Name",
                            })}
                          </label>
                          <Input
                            value={editableFields.funnelName}
                            readOnly
                            className="bg-card border-border text-muted-foreground"
                          />
                        </div>
                    ...
                    <div className="flex-1 bg-muted border border-border rounded-xl p-4 overflow-y-auto max-h-[600px] space-y-4 custom-scrollbar">
                      {selectedLead.activities?.length === 0 ? (
                        <p className="text-center text-muted-foreground text-sm py-8">
                          {t("crm.modal.noActivities", {
                            defaultValue: "No activities recorded yet",
                          })}
                        </p>
                      ) : (
                        [...selectedLead.activities]
                          ...
                          .map((activity, idx) => (
                            <div
                              key={idx}
                              className="relative pl-6 pb-4 border-l-2 border-slate-800 last:pb-0"
                            >
                              <div
                                className={`absolute -left-1.5 top-1 w-3 h-3 rounded-full ${activityColor(activity.activityType)}`}
                              />
                              <div className="flex items-center justify-between mb-1">
                                ...
                                <span className="text-[10px] bg-slate-800 px-1.5 py-0.5 rounded text-slate-400 font-bold">
                                  {t(
                                    `crm.activityType.${activity.activityType?.toLowerCase()}`,
