...
const ChatSupport = () => {
  ...
  const [humanFormData, setHumanFormData] = useState({
    name: "",
    email: "",
    message: "",
  });
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Only ever add the bot reply. Never auto-open Human Support form from backend
      // (when OPENAI_API_KEY is missing, backend returns transfer_to_human: true on every message).
      setMessages((prev) => [
        ...prev,
        {
          type: "bot",
    ...
    <>
      ...
      {!isOpen && (
        ...
        >
          ...
          <span className="absolute -top-1 -right-1 w-5 h-5 bg-emerald-500 rounded-full flex items-center justify-center text-white text-[10px] font-bold border-2 border-white">
            1
          </span>
        </button>
      )}
        ...
        >
          ...
          <div
            className={`px-4 py-3 flex items-center justify-between border-b flex-shrink-0 ${
              isDark
                ? "bg-gradient-to-r from-[#12161d] to-[#0d1117] border-white/10"
                  ...
                  >
                    ...
                    >
                      <input
                        ...
                        required
                        className={`w-full px-3 py-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50 ${
                          isDark
                      ...
                      <textarea
                        ...
                        required
                        rows={2}
                        className={`w-full px-3 py-2 rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-amber-500/50 ${
                          isDark
                            ? "bg-[#0a0d12] border border-white/10 text-white placeholder-gray-500"
                            : "bg-slate-50 border border-slate-200 text-slate-900 placeholder-slate-400"
                        }`}
                  ...
                  <div className="flex flex-wrap gap-1.5 items-center">
                    ...
                    {messages.length < 4 && (
                      <>
                        <button
                          onClick={() =>
                            handleQuickReply("What challenges do you offer?")
                          }
                          className={`text-xs px-2.5 py-1.5 rounded-full transition-colors border font-medium ${
                            isDark
                        ...
                        <button
                          onClick={() =>
                            handleQuickReply("How do payouts work?")
                          }
                          className={`text-xs px-2.5 py-1.5 rounded-full transition-colors border font-medium ${
                            isDark
                              ? "bg-amber-500/20 text-amber-400 border-amber-500/40 hover:bg-amber-500/30"
                    ...
                    <button
                      ...
                      className={`text-xs px-2.5 py-1.5 rounded-full transition-colors border flex items-center gap-1.5 ml-auto ${
                        isDark
              ...
              >
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className={`flex-1 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50 ${
                      isDark
