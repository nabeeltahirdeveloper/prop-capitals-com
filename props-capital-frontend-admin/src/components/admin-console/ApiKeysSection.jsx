...
export default function ApiKeysSection() {
  ...
  const handleToggleStatus = async (key) => {
    ...
    if (newStatus === 'revoked' && !confirm(`Revoke API key for "${key.project_name}"? This will block all requests from this project.`)) return;
    try {
  ...
  const handleDelete = async (key) => {
    if (!confirm(`Permanently delete API key for "${key.project_name}"? This cannot be undone.`)) return;
    try {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-3xl font-bold gradient-text">API Keys</h2>
          <p className="text-gray-400 mt-1">Manage API keys for external projects using the Checkout API</p>
        </div>
        <button
          className="action-btn btn-primary"
          onClick={() => setModalState({ show: true, mode: 'create', item: null })}
        >
          <i className="fas fa-plus mr-2"></i>Generate Key
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="glass-panel p-12 text-center">
          ...
          <p className="text-gray-500 mt-2">Generate a key to allow external projects to use the Checkout API</p>
        </div>
      ) : (
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex gap-4 text-xs text-gray-500">
                    <span><i className="fas fa-calendar mr-1"></i>Created: {formatDate(key.created_at)}</span>
                    <span><i className="fas fa-clock mr-1"></i>Last used: {formatDate(key.last_used_at)}</span>
                  </div>
                </div>
