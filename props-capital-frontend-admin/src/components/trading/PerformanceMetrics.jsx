...
export default function PerformanceMetrics({ metrics }) {
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: TrendingUp,
      color:
        profitFactor >= 1 || (losingTrades === 0 && winningTrades > 0)
          ? "text-emerald-400"
          : profitFactor > 0
            ? "text-red-400"
            : "text-slate-400",
      ...
      <div className="mb-6">
        <div className="flex justify-between text-sm mb-2">
          <span className="text-emerald-400">
            {winningTrades} {t("analytics.wins")}
          </span>
          <span className="text-red-400">
            {losingTrades} {t("analytics.losses")}
          </span>
        </div>
        {totalTrades > 0 ? (
