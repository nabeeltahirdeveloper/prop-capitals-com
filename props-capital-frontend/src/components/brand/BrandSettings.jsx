...
function SecuritySection() {
  ...
  const handleUpdatePassword = async (e) => {
    ...
    setLoading(true);
    try {
      await brandApi.auth.changePassword(currentPassword, newPassword);
...
function SettlementsSection() {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        setLoading(true);
        const res = await brandApi.profile.get();
        const settings = res?.brand || res || {};
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await brandApi.profile.update(settlementData);
