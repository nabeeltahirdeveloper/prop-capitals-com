...
export default function BrandVisitsSection() {
  ...
  const loadVisits = async () => {
    setLoading(true);
    try {
      const data = await brandApi.visits.list({ page: String(page), pageSize: '20' });
  ...
  const loadStats = async () => {
    try {
      const data = await brandApi.visits.getStats({ days: String(days) });
      setStats(data);
    } catch (error) {
  ...
  const loadLinks = async () => {
    try {
      const data = await brandApi.links.list();
      setLinks(data.links || []);
    } catch (error) {
