...
export default function BrandAllTransactionsSection() {
  ...
  const loadAllTransactions = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const result = await brandApi.allTransactions.list(params);
      setData(result);
    } catch (error) {
                          ...
                          <div className="mt-6 pt-6 border-t border-gray-300">
                            ...
                            <p className={`text-base p-3 rounded-lg border whitespace-pre-line ${
                              tx.payment_status === 'paid'
                                ...
                                : tx.payment_status === 'declined' || tx.payment_status === 'decline' || tx.payment_status === 'failed' || tx.payment_status === 'unpaid'
                                  ? 'text-red-800 bg-red-50/50 border-red-200/50'
                                  : 'text-yellow-800 bg-yellow-50/50 border-yellow-200/50'
                            }`}>
                              {tx.payment_status === 'paid'
                                ? `✅ Transaction successfully approved${tx.payment_message ? '\n' + transformPaymentMessage(tx.payment_message) : ''}`
                                : tx.payment_status === 'declined' || tx.payment_status === 'decline' || tx.payment_status === 'failed' || tx.payment_status === 'unpaid'
                                  ? `❌ Transaction declined${tx.payment_message ? '\nReason: ' + transformPaymentMessage(tx.payment_message) : ''}`
                                  : `⏳ Transaction pending${tx.payment_message ? '\n' + transformPaymentMessage(tx.payment_message) : ''}`}
                            </p>
                          </div>
