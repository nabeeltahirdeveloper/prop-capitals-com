...
export default function DailyPnLChart({ data }) {
  ...
  const totalProfit = chartData.reduce((sum, d) => d.pnl > 0 ? sum + d.pnl : sum, 0);
  const totalLoss = chartData.reduce((sum, d) => d.pnl < 0 ? sum + d.pnl : sum, 0);
    ...
    <Card className="bg-slate-900 border-slate-800 p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-6 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-400">{losingDays} {t('analytics.losingDays')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mt-4">
        <div className="bg-slate-800/50 rounded-lg p-3">
          <p className="text-xs text-slate-400 mb-1">{t('analytics.bestDay')}</p>
          <p className="text-emerald-400 font-bold">
            +${Math.max(...chartData.map(d => d.pnl)).toLocaleString()}
          </p>
        </div>
        <div className="bg-slate-800/50 rounded-lg p-3">
          <p className="text-xs text-slate-400 mb-1">{t('analytics.worstDay')}</p>
