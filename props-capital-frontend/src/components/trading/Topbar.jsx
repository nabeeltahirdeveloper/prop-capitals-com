...
const TopBar = ({ selectedSymbol, selectedTimeframe, onTimeframeChange, chartType = 'candles', onChartTypeChange, onNewOrder, onZoomIn, onZoomOut, onToggleBuySell, onDownloadChartPNG, onToggleFullscreen, marketWatchOpen = true, onToggleMarketWatch, buySellPanelOpen }) => {
    ...
    const isDark = traderTheme?.isDark ?? (theme === 'dark')
    ...
    return (
        <div className={`flex items-center justify-between px-4 py-1 border-b ${isLight ? 'bg-white border-slate-200' : 'bg-[#12161d] border-white/5'}`}>
            {/* Left: Chart Type + Timeframes */}
            <div className='flex items-center gap-4'>
                {/* Chart type: Bars, Candles, Line, Area (LEFT side) */}
                <div className={`flex items-center gap-1 border-r pr-3 ${isLight ? 'border-slate-200' : 'border-white/5'}`}>
                    {['bars', 'candles', 'line', 'area'].map((type) => (
                        <button
                            ...
                            className={`px-1 py-1.5 text-[11px] font-medium rounded transition-colors capitalize flex items-center gap-1.5 
                                ${chartType === type
                                    ? (isLight ? 'text-secondary' : 'text-amber-500')
                ...
                <div className={`flex items-center gap-1 border-r ${isLight ? 'border-slate-200' : 'border-white/5'} pr-1`}>
                ...
                <div className='flex items-center gap-2'>
                    ...
                    <div className={`border-r h-6 mx-1 ${isLight ? 'border-slate-200' : 'border-white/5'}`}></div>
                    ...
                    <button
                        onClick={onNewOrder}
                        className={`px-2 py-2 flex justify-center items-center gap-1 cursor-pointer text-xs font-medium rounded transition-colors ${isLight ? 'text-secondary hover:bg-[#E5F4FF]' : 'text-white hover:bg-white/5'}`}
                    >
                        <svg width={24} height={24} viewBox='0 0 24 24 ' xmlns='http://www.w3.org/2000/svg'>
                            <g fill='none' fillRule="evenodd">
                    ...
                    {/* Border line after Order Button */}
                     <div className={`border-r ${isLight ? 'border-slate-200' : 'border-white/5'} h-6 mx-1`}></div> 
                    ...
                    <div className='flex items-center gap-1'>
                        ...
                        {/* Add Indicator */}
                         <button
                            onClick={onNewOrder}
                            className={`px-2 py-2 flex justify-center items-center gap-1 cursor-pointer text-xs font-medium rounded transition-colors ${isLight ? 'text-slate-600 hover:text-sky-600 hover:bg-[#E5F4FF]' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                        >
                        ...
                        {/* Economic calendar */}
                         <button
                            onClick={onNewOrder}
                            className={`px-2 py-2 flex justify-center items-center gap-1 cursor-pointer text-xs font-medium rounded transition-colors ${isLight ? 'text-secondary hover:bg-[#E5F4FF]' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                        >
