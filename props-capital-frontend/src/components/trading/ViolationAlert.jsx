...
export default function ViolationAlert({ account, onDismiss }) {
  ...
  if (!account.violation_reason || account.status !== 'failed') {
    return null;
  }
  ...
  const violationDetails = {
    'daily_drawdown': {
      ...
      description: t('violationAlert.dailyDrawdownDescription', { limit: account.challenge?.dailyDrawdownPercent || 5 }),
      color: 'red',
    ...
    'overall_drawdown': {
      ...
      description: t('violationAlert.maximumDrawdownDescription', { limit: account.challenge?.overallDrawdownPercent || 10 }),
      color: 'red',
  ...
  // Detect violation type from reason
  const violationType = account.violation_reason?.toLowerCase().includes('daily') ? 'daily_drawdown' :
                        account.violation_reason?.toLowerCase().includes('overall') || account.violation_reason?.toLowerCase().includes('max') ? 'overall_drawdown' :
                        account.violation_reason?.toLowerCase().includes('day') ? 'max_trading_days' : 'overall_drawdown';
      ...
      <div className="relative">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-xl font-bold text-red-400">{details.title}</h3>
            </div>
          </div>
          {onDismiss && (
            <button 
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-6">
          <div className="bg-slate-900/50 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm text-slate-400">{t('violationAlert.valueAtViolation')}</span>
            </div>
          ...
          <div className="bg-slate-900/50 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm text-slate-400">{t('violationAlert.maximumAllowed')}</span>
            </div>
        ...
        <div className="bg-slate-900/50 rounded-lg p-4 mb-6">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-sm text-slate-400">{t('violationAlert.account')}</p>
              <p className="text-white font-medium">#{account.account_number}</p>
            </div>
            <div>
              <p className="text-sm text-slate-400">{t('violationAlert.finalBalance')}</p>
              <p className="text-white font-medium">${account.current_balance?.toLocaleString()}</p>
            </div>
            <div>
              <p className="text-sm text-slate-400">{t('violationAlert.tradingDays')}</p>
              <p className="text-white font-medium">{account.trading_days_count}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Link to={createPageUrl('Rules')} className="flex-1">
            <Button variant="outline" className="w-full border-slate-700 bg-slate-800/50 text-white hover:bg-slate-700">
