...
import {
  compressForexCandles,
  formatCompressedTime,
  processCandles,
  timeframeToSeconds,
} from "../../utils/candleEngine";
...
const prepareCandlesForSetData = (candles, selectedTimeframe) => {
  ...
  const aligned = (Array.isArray(candles) ? candles : [])
    .map((c) => {
      const t = normalizeChartTime(c.time); // handles sec/ms + null safety
      return {
        ...c,
...
const ChartArea = forwardRef(function ChartArea(
  ...
  const forexIndexToRealTimeMapRef = useRef(new Map()); // logicalIndex -> real epoch seconds
  const forexRealTimeToIndexMapRef = useRef(new Map()); // real epoch seconds -> logicalIndex
  const forexLastIndexRef = useRef(0);
  const isForexCompressedRef = useRef(false);
  ...
  const isForexInstrument = isForexSymbol(selectedSymbolStr);

  const resetForexCompressionMaps = () => {
    forexIndexToRealTimeMapRef.current = new Map();
    forexRealTimeToIndexMapRef.current = new Map();
    forexLastIndexRef.current = 0;
  };

  const syncForexCompressionMaps = (sourceCandles) => {
    if (!isForexSymbol(selectedSymbolStr)) {
      resetForexCompressionMaps();
      return [];
    }
    const prepared = prepareCandlesForSetData(sourceCandles, selectedTimeframe);
    const compression = compressForexCandles(prepared);
    forexIndexToRealTimeMapRef.current = compression.indexToRealTimeMap;
    forexRealTimeToIndexMapRef.current = compression.realTimeToIndexMap;
    forexLastIndexRef.current = compression.lastIndex;
    return compression.compressedCandles;
  };

  const resolveRealTimeFromChartTime = (chartTime) => {
    const normalized = normalizeChartTime(chartTime);
    if (normalized == null) return null;
    if (!isForexCompressedRef.current) return normalized;
    return forexIndexToRealTimeMapRef.current.get(normalized) ?? null;
  };

  const getOrCreateChartTimeForRealTime = (realEpochSeconds) => {
    const normalizedReal = normalizeTime(realEpochSeconds);
    if (normalizedReal == null) return null;
    if (!isForexCompressedRef.current) return normalizedReal;

    const existingIndex =
      forexRealTimeToIndexMapRef.current.get(normalizedReal);
    if (existingIndex != null) return existingIndex;

    const nextIndex = forexLastIndexRef.current + 1;
    forexLastIndexRef.current = nextIndex;
    forexRealTimeToIndexMapRef.current.set(normalizedReal, nextIndex);
    forexIndexToRealTimeMapRef.current.set(nextIndex, normalizedReal);
    return nextIndex;
  };

  const formatChartTimeLabel = (time, tickMarkType, locale) => {
    const real = resolveRealTimeFromChartTime(time);
    if (real != null) {
    ...
    const formatted = isForexCompressedRef.current
      ? formatCompressedTime(
          time,
          forexIndexToRealTimeMapRef.current,
          tickMarkType,
          locale,
        )
      : "";
    if (formatted) return formatted;
    const fallback = normalizeChartTime(time);
    return fallback == null ? "" : String(fallback);
  };

  const formatCrosshairTimeLabel = (time) => {
    const real = resolveRealTimeFromChartTime(time);
    if (real == null) {
      const fallback = normalizeChartTime(time);
      return fallback == null ? "" : String(fallback);
    ...
    const pad = (n) => String(n).padStart(2, "0");
    return `${pad(d.getUTCDate())} ${d.toLocaleDateString("en", { month: "short" })} '${String(d.getUTCFullYear()).slice(-2)} ${pad(d.getUTCHours())}:${pad(d.getUTCMinutes())}`;
  };
  ...
  useEffect(() => {
    isForexCompressedRef.current = isForexInstrument;
    if (!isForexInstrument) {
      resetForexCompressionMaps();
    } else {
  ...
  useEffect(() => {
    ...
    const chart = createChart(container, {
      ...
      localization: {
        priceFormatter,
        timeFormatter: (time) => formatCrosshairTimeLabel(time),
      },
      layout: {
      ...
      timeScale: {
        ...
        tickMarkFormatter: (time, tickMarkType, locale) => {
          return formatChartTimeLabel(time, tickMarkType, locale);
        },
      },
  ...
  useEffect(() => {
    const fetchCandles = async () => {
      ...
      try {
        ...
        const rawCandles = await getMarketHistory(
          selectedSymbolStr,
          selectedTimeframe,
          limit,
        );
        ...
        if (rawCandles && rawCandles.length > 0) {
          ...
          if (import.meta.env?.DEV) {
            ...
            console.log(
              "MOD60",
              selectedSymbolStr,
              rawCandles.slice(0, 5).map((c) => normalizeTime(c.time) % 60),
            );
          }
          ...
          if (processedCandles.length > 0) {
            ...
            console.warn(
              `⚠️ No valid candles after processing for ${selectedSymbolStr}`,
            );
            setCandles([]);
            lastHistTimeRef.current = 0;
          }
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      const bidNum =
        typeof bid === "number" && !isNaN(bid) ? bid : parseFloat(bid);
      const askNum =
        typeof ask === "number" && !isNaN(ask) ? ask : parseFloat(ask);
      const price =
      ...
      const chartTime = getOrCreateChartTimeForRealTime(barTime);
      if (chartTime == null) return;

      try {
        if (chartType === "line" || chartType === "area") {
          s.update({ time: chartTime, value: price });
        } else {
          s.update({ time: chartTime, open, high, low, close: price });
        }
        candlesMapRef.current.set(barTime, {
          time: barTime,
          open,
          high,
          low,
  ...
  useEffect(() => {
    ...
    if (!hasInitializedRef.current) {
      ...
      if (chartType === "line" || chartType === "area") {
        // For line/area charts, extract close prices from display candles
        const lineAreaData = displayCandles
          .map((c) => ({
            time: c.time,
            value: c.close,
          }))
        ...
        // For candlestick charts: display candles are already prepared/compressed as needed
        const finalCandles = displayCandles;
  ...
  useEffect(() => {
    ...
    if (chartType === "bars") {
      ...
      chart.priceScale("right").applyOptions({
        scaleMargins: { top: 0.05, bottom: 0.3 }, // candles top/bottom space
      });

      chart.priceScale("vol").applyOptions({
        scaleMargins: { top: 0.75, bottom: 0.0 }, // volume at bottom
          ...
          if (displayCandles.length > 0) {
            try {
              ...
              if (lastCandleTime == null)
                throw new Error("missing-last-candle-time");
              const barsBack = isForexCompressedRef.current ? 100 : 100 * 60;
              const firstVisibleTime = Math.max(1, lastCandleTime - barsBack); // Show last window without invalid epoch-style shift on compressed indices
              timeScale.setVisibleRange({
  ...
  }, [candles, chartType, selectedTimeframe, displayCandles]); // Update when candles, chartType or timeframe changes
  ...
  useEffect(() => {
    ...
    if (toolType === "trendline") {
      ...
      const handleTrendLineMouseMove = (e) => {
        if (
          trendLineFirstPointRef.current === null ||
          currentDrawingRef.current?.type !== "trendline"
        )
          return;
        e.preventDefault();
        e.stopPropagation();
      ...
      return () => {
        ...
        if (
          currentDrawingRef.current?.type === "trendline" &&
          currentDrawingRef.current?.series
        ) {
          try {
            chart.removeSeries(currentDrawingRef.current.series);
          } catch (err) {}
    ...
    if (toolType === "parallellines") {
      ...
      const handleParallelChannelClick = (e) => {
        ...
        if (pts.length === 0) {
          ...
          const parPreview = computeParallelLineFromThreePoints(
            chart,
            series,
            pts[0],
            point,
          ...
          if (parPreview) {
            currentDrawingRef.current.previewLine.setData(
          ...
          if (!baseLine) return;
          const parPoints = computeParallelLineFromThreePoints(
            chart,
            series,
            p1,
            p2,
    ...
    const handleMouseMove = (e) => {
      ...
      if (currentDrawingRef.current.type === "rectangle") {
        ...
        const leftTime = normalizeChartTime(Math.min(start.time, time));
        const rightTime = normalizeChartTime(Math.max(start.time, time));
        if (leftTime == null || rightTime == null) return;
        if (
          Math.abs(rightTime - leftTime) < 1 ||
          Math.abs(topPrice - bottomPrice) < 0.00001
        )
          return;
    ...
    const handleMouseUp = (e) => {
      ...
      if (currentDrawingRef.current.type === "rectangle") {
        ...
        const leftTime = normalizeChartTime(Math.min(start.time, end.time));
        const rightTime = normalizeChartTime(Math.max(start.time, end.time));
        if (
          leftTime != null &&
          rightTime != null &&
          Math.abs(rightTime - leftTime) >= 1 &&
    ...
    if (
      ...
      container.addEventListener(
        ...
        handleMouseMovePreventPan,
        options,
      ); // Prevent chart pan on mouse move
  ...
  useEffect(() => {
    ...
    const handleSelectionMouseMove = (e) => {
      ...
      if (dragModeRef.current === "handle1") {
        ...
        const newPoint1 = {
          time: normalizeChartTime(start.point1.time + deltaTime),
          price: start.point1.price + deltaPrice,
        };
        const newPoint2 = {
          time: normalizeChartTime(start.point2.time + deltaTime),
  ...
  useEffect(() => {
    ...
    drawingObjectsRef.current.forEach((obj) => {
        ...
        obj.priceLine.applyOptions(
          drawingsVisible
            ? {
                ...
                axisLabelVisible: true,
                title:
                  typeof obj.price === "number" ? obj.price.toFixed(5) : "",
              }
            : {
                color: "rgba(0,0,0,0)",
      ...
      if (obj.type === "parallellines") {
        if (obj.baseLine)
          obj.baseLine.applyOptions({ visible: drawingsVisible });
        if (obj.parallelLine)
          obj.parallelLine.applyOptions({ visible: drawingsVisible });
        if (obj.firstLine)
          obj.firstLine.applyOptions({ visible: drawingsVisible });
        if (obj.secondLine)
          obj.secondLine.applyOptions({ visible: drawingsVisible });
      }
    });
  ...
  useEffect(() => {
    if (drawingObjectsRef.current.length > chartObjects.length) {
      ...
      if (
        selectedChartObject &&
        removed.some((obj) => obj === selectedChartObject)
      ) {
        setSelectedChartObject(null);
        setHandlePositions([]);
      }
      removed.forEach((obj) => {
        ...
        if (obj.type === "parallellines") {
          try {
            ...
            if (obj.parallelLine)
              chartRef.current?.removeSeries(obj.parallelLine);
  ...
  return (
    <div
      className={`flex-1 flex flex-col h-full overflow-hidden ${isDark ? "bg-[#0d1117] border-white/10" : "bg-white border-slate-200"}`}
    >
