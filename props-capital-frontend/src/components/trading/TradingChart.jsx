...
export default function TradingChart({ symbol, openPositions = [], onPriceUpdate }) {
  ...
  const getPriceFormatConfig = useCallback(() => {
    ...
    if (symbolUpper.includes('EUR') || symbolUpper.includes('AUD') || 
        symbolUpper.includes('GBP') || symbolUpper.includes('NZD') ||
    ...
    if (symbolUpper.includes('XRP') || symbolUpper.includes('ADA') || 
  ...
  useEffect(() => {
    ...
    const checkAndInitialize = () => {
      ...
      if (width > 0 && height > 0 && !chartRef.current) {
        console.log('Container ready, initializing chart...', { width, height });
        initializeChart();
    ...
    function initializeChart() {
      ...
      if (width <= 0 || height <= 0 || isNaN(width) || isNaN(height)) {
        console.error('Invalid container dimensions:', { width, height, clientWidth: container.clientWidth, clientHeight: container.clientHeight });
        return;
      }
      ...
      requestAnimationFrame(() => {
        setTimeout(() => {
          ...
          try {
            console.log('Adding candlestick series...', { chartWidth: width, chartHeight: height, priceFormat: priceFormatConfig });
            ...
            setError(`Failed to create chart series: ${err.message || 'Unknown error'}`);
            return;
          }
          ...
          try {
            ...
            const volumeSeries = chart.addSeries(HistogramSeries, {
              ...
              scaleMargins: {
                top: 0.90, // Volume occupies bottom 10% - fades into background (1 - 0.90 = 0.10)
                bottom: 0.0,
              },
            });
            ...
            console.warn('Failed to create volume series, continuing without it:', err);
  ...
  const fetchHistory = useCallback(async (symbolName, tf) => {
    if (!symbolName || !chartRef.current || !candlestickSeriesRef.current || !lineSeriesRef.current) {
    ...
    try {
      ...
      if (!historyData || (Array.isArray(historyData) && historyData.length === 0)) {
        setError('No data available. The API may be rate-limited. Please wait a moment and try again.');
        setIsLoading(false);
        return;
      }
      ...
      if (Array.isArray(historyData) && historyData.length > 0) {
        ...
        console.log(`[TradingChart] Received ${historyData.length} candles for ${symbolName} ${tf}`);
        if (historyData.length > 0) {
          const first = historyData[0];
          const last = historyData[historyData.length - 1];
          console.log(`[TradingChart] First candle time: ${first.time} (${new Date(first.time * 1000).toISOString()})`);
          console.log(`[TradingChart] Last candle time: ${last.time} (${new Date(last.time * 1000).toISOString()})`);
        }
          ...
          .map(candle => {
            ...
            if (candle.time) {
              ...
              if (typeof candle.time === 'number') {
                ...
                timeValue = candle.time < 946684800000 ? candle.time : Math.floor(candle.time / 1000);
              } else {
                timeValue = Math.floor(new Date(candle.time).getTime() / 1000);
              }
            } else if (candle.timestamp) {
              if (typeof candle.timestamp === 'number') {
                timeValue = candle.timestamp < 946684800000 ? candle.timestamp : Math.floor(candle.timestamp / 1000);
              } else {
                timeValue = Math.floor(new Date(candle.timestamp).getTime() / 1000);
              }
            } else {
            ...
            const open = typeof candle.open === 'number' && !isNaN(candle.open) ? candle.open : (candle.close || 0);
            const high = typeof candle.high === 'number' && !isNaN(candle.high) ? candle.high : (candle.close || open);
            const low = typeof candle.low === 'number' && !isNaN(candle.low) ? candle.low : (candle.close || open);
            const close = typeof candle.close === 'number' && !isNaN(candle.close) ? candle.close : open;
        ...
        for (const candle of historyData) {
          ...
          if (matchingCandle) {
            ...
            volumeData.push({
              time: utcTime,
              value: volume,
              color: isUp ? 'rgba(38, 166, 154, 0.24)' : 'rgba(239, 83, 80, 0.24)', // Reduced opacity for visual calm
            });
          }
        }
        ...
        const validVolumeData = volumeData.filter(v => 
          v && 
        ...
        const updateChart = () => {
          try {
            ...
            if (chartType === 'candle' && candlestickSeriesRef.current && finalCandles.length > 0) {
              console.log(`[TradingChart] Setting candlestick data for ${symbol?.symbol} ${tf}: ${finalCandles.length} candles`);
              
              // Frontend validation: Log last 5 candles to check structure
              const last5 = finalCandles.slice(-5);
              console.log('[TradingChart] Last 5 candles structure:', last5.map(c => ({
                time: c.time,
                open: c.open,
                high: c.high,
                low: c.low,
                ...
                allNumeric: typeof c.open === 'number' && typeof c.high === 'number' && typeof c.low === 'number' && typeof c.close === 'number',
                isFlat: c.open === c.high && c.high === c.low && c.low === c.close,
              })));
              
              // Verify first candle is valid
              const firstCandle = finalCandles[0];
              if (firstCandle && typeof firstCandle.time === 'number' && firstCandle.time > 0) {
                // Ensure we're passing the correct structure (not line data)
                const validCandles = finalCandles.map(c => {
                  // If it has 'value' instead of OHLC, skip it
                  if ('value' in c && !('open' in c)) {
                    console.warn('[TradingChart] Invalid candle structure (has value, missing OHLC):', c);
                    return null;
                  }
                  // Ensure all OHLC fields are numeric
                  if (typeof c.open !== 'number' || typeof c.high !== 'number' || typeof c.low !== 'number' || typeof c.close !== 'number') {
                    console.warn('[TradingChart] Invalid candle structure (non-numeric OHLC):', c);
                    return null;
                  }
                  return {
                    time: c.time,
                ...
                if (validCandles.length > 0) {
                  ...
                  console.error('[TradingChart] No valid candles after validation');
                }
              } else {
                console.error('Invalid first candle:', firstCandle);
              }
            } else if (chartType === 'line' && lineSeriesRef.current && finalLineData.length > 0) {
              console.log('Setting line data:', finalLineData.length, 'points');
            ...
            console.error('Candles data sample (first 3):', finalCandles.slice(0, 3));
            console.error('First candle details:', finalCandles[0]);
            setError(`Failed to render chart: ${err.message || 'Unknown error'}`);
          }
        };
        ...
        if (finalCandles.length > 0) {
          ...
          const lastVolume = validVolumeData.length > 0 
        ...
        if (onPriceUpdate && finalCandles.length > 0) {
          ...
          else if (symbolName.includes('BTC') || symbolName.includes('ETH') || symbolName.includes('SOL')) decimals = 2;
          else if (symbolName.includes('XRP') || symbolName.includes('ADA') || symbolName.includes('DOGE')) decimals = 4;

          const formattedPrice = parseFloat(lastCandle.close.toFixed(decimals));
      ...
      if (err?.message?.includes('Too Many Requests') || err?.message?.includes('rate limit')) {
        ...
        setError(`Symbol ${symbolName} not found. Please check the symbol name.`);
      } else {
  ...
  useEffect(() => {
    ...
    if (symbol?.symbol && chartRef.current && candlestickSeriesRef.current && lineSeriesRef.current && seriesReadyRef.current) {
      ...
      debounceTimerRef.current = setTimeout(() => {
        if (candlestickSeriesRef.current && lineSeriesRef.current && seriesReadyRef.current) {
          fetchHistory(symbol.symbol, timeframe);
        }
      }, 500);
  ...
  const updateCurrentPriceLine = useCallback((price) => {
    ...
    if (chartType === 'candle' && candlestickSeriesRef.current) {
      currentPriceLineRef.current = candlestickSeriesRef.current.createPriceLine(priceLine);
    } else if (chartType === 'line' && lineSeriesRef.current) {
      currentPriceLineRef.current = lineSeriesRef.current.createPriceLine(priceLine);
    }
  ...
  useEffect(() => {
    ...
    const checkAndStartUpdates = () => {
      if (!seriesReadyRef.current || !candlestickSeriesRef.current || !lineSeriesRef.current) {
      ...
      const updateChartFromUnifiedPrices = () => {
        ...
        if (priceData !== null && priceData !== undefined && !isNaN(priceData) && priceData > 0) {
          ...
          if (onPriceUpdate) {
            ...
            else if (symbol.symbol.includes('BTC') || symbol.symbol.includes('ETH') || symbol.symbol.includes('SOL')) decimals = 2;
            else if (symbol.symbol.includes('XRP') || symbol.symbol.includes('ADA') || symbol.symbol.includes('DOGE')) decimals = 4;
          ...
          if (currentCandles.length > 0 || currentLineData.length > 0) {
            const lastCandleTime = currentCandles.length > 0 
            ...
            if (currentTime - lastCandleTime >= (tfMs / 1000)) {
              const openPrice = currentPriceRef.current > 0 ? currentPriceRef.current : newPrice;
              const newCandle = {
                time: currentTime,
                open: openPrice,
                close: newPrice,
              ...
              const newVolumePoint = {
                time: currentTime,
                value: 0,
                color: newPrice >= openPrice ? 'rgba(38, 166, 154, 0.24)' : 'rgba(239, 83, 80, 0.24)',
              };
              ...
              try {
                ...
                console.error(`[Chart] Error updating chart for ${symbol.symbol}:`, err);
              }
            } else {
              // Aggregate tick into current candle
              const lastCandle = currentCandles.length > 0 
                ? currentCandles[currentCandles.length - 1] 
                : null;

              if (lastCandle && typeof lastCandle.time === 'number' && !isNaN(lastCandle.time)) {
                const lastHigh = typeof lastCandle.high === 'number' && !isNaN(lastCandle.high) ? lastCandle.high : newPrice;
                const lastLow = typeof lastCandle.low === 'number' && !isNaN(lastCandle.low) ? lastCandle.low : newPrice;
                const lastOpen = typeof lastCandle.open === 'number' && !isNaN(lastCandle.open) ? lastCandle.open : newPrice;
                ...
                const updatedVolumePoint = {
                  ...
                  color: newPrice >= lastOpen ? 'rgba(38, 166, 154, 0.24)' : 'rgba(239, 83, 80, 0.24)',
                };
                ...
                try {
                  ...
                  console.error(`[Chart] Error updating existing candle/line for ${symbol.symbol}:`, err);
                }
              }
            }
          } else {
            ...
            try {
              ...
              console.log(`[Chart] Cannot initialize empty chart yet for ${symbol.symbol}, waiting for data...`);
            }
          }
        }
      };
  ...
  }, [symbol?.symbol, timeframe, chartType, onPriceUpdate, updateCurrentPriceLine, getUnifiedPrice]);
  ...
  useEffect(() => {
    ...
    const relevantPositions = openPositions.filter(p => p.symbol === symbol.symbol);
  ...
  const getPriceChange = useCallback(() => {
    ...
    if (data.length < 2) return { change: 0, percent: 0, lastPrice: currentPriceRef.current };
  ...
  return (
    <Card className="bg-slate-900 border-slate-800 p-3 sm:p-4 h-full flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
        <div>
          <h3 className="text-base sm:text-lg font-bold text-white">{symbol?.symbol || 'EUR/USD'}</h3>
          <span className={`text-xs sm:text-sm font-mono ${
            priceInfo.percent >= 0 ? 'text-emerald-400' : 'text-red-400'
          }`}>
            {formatPrice(priceInfo.lastPrice)} 
            <span className="ml-2">
              {priceInfo.percent >= 0 ? '+' : ''}{priceInfo.percent.toFixed(2)}%
            </span>
          </span>
        </div>
      ...
      <div className="flex-1 relative bg-[#131722] rounded-lg overflow-hidden min-h-[300px]">
        {isLoading && (
          <div className="absolute inset-0 flex items-center justify-center z-10 bg-[#131722]/80">
            <div className="text-[#d1d4dc] text-sm">{t('terminal.chart.loadingData')}</div>
          </div>
        )}
      ...
      <div className="mt-2 flex items-center justify-between text-xs text-slate-500">
        <div className="flex items-center gap-2">
          ...
          <span>{t('terminal.chart.lastUpdate')}: {new Date(lastUpdateTimeRef.current).toLocaleTimeString()}</span>
        </div>
        <div className="flex items-center gap-4">
          <span>{t('terminal.chart.tf')}: {timeframe}</span>
          {isLoading && <span className="text-emerald-400">{t('terminal.chart.updating')}</span>}
        </div>
      </div>
    </Card>
  );
