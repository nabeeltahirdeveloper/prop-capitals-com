...
export default function ChallengeRulesPanel({ account, challenge }) {
  ...
  useEffect(() => {
    ...
    const profitTarget = parseFloat(
      challenge?.phase1TargetPercent || account.profitTarget || 10,
    );
    const maxDailyLoss = parseFloat(
      challenge?.dailyDrawdownPercent || account.maxDailyDrawdown || 5,
    );
    const maxOverallDrawdown = parseFloat(
      challenge?.overallDrawdownPercent || account.maxOverallDrawdown || 10,
    );
    ...
    <Card className="bg-slate-900 border-slate-800 p-6">
      ...
      <div className="space-y-4 mb-6">
        ...
        <div
          className={`p-4 rounded-lg border ${
            profitPercent >= profitTarget
              ...
              : profitPercent >= profitTarget * 0.7
            ...
            <Badge
              className={
                profitPercent >= profitTarget
                  ...
                  : profitPercent >= profitTarget * 0.7
                    ? "bg-amber-500/20 text-amber-400 border-amber-500/30 pointer-events-none"
                    : profitPercent < 0
                      ? "bg-red-500/20 text-red-400 border-red-500/30 pointer-events-none"
                      : "bg-slate-500/20 text-slate-400 border-slate-500/30 pointer-events-none"
              }
            >
              {profitPercent >= profitTarget
                ? t("dashboard.rulesPanel.status.reached")
                : profitPercent < 0
                  ? t("dashboard.rulesPanel.status.loss")
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            >
              {dailyLoss >= maxDailyLoss
                ? t("dashboard.rulesPanel.status.violated")
                : dailyLossProgress >= 80
                  ? t("dashboard.rulesPanel.status.danger")
                  : dailyLossProgress >= 50
                    ? t("dashboard.rulesPanel.status.warning")
                    : t("dashboard.rulesPanel.status.safe")}
            </Badge>
          </div>
          <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
            <div
              className={
                dailyLoss >= maxDailyLoss || dailyLossProgress >= 80
                  ? "h-full bg-red-500"
                  : dailyLossProgress >= 50
                    ? "h-full bg-amber-500"
                    : "h-full bg-emerald-500"
          ...
          <div className="flex justify-between mt-2 text-xs">
            ...
            <span className="text-slate-400">
              {t("dashboard.rulesPanel.limit")}: {maxDailyLoss}%
            </span>
          </div>
        </div>
        ...
        <div
          className={`p-4 rounded-lg border ${
            overallDrawdown >= maxOverallDrawdown
              ? "bg-red-500/10 border-red-500/30"
              : overallDrawdownProgress >= 80
                ? "bg-red-500/10 border-red-500/30"
                : overallDrawdownProgress >= 50
                  ? "bg-amber-500/10 border-amber-500/30"
                  : "bg-emerald-500/10 border-emerald-500/30"
          }`}
        >
          <div className="flex items-center justify-between mb-2">
            ...
            <Badge
              className={
                overallDrawdown >= maxOverallDrawdown
                  ? "bg-red-500/20 text-red-400 border-red-500/30 border-none pointer-events-none "
                  : overallDrawdownProgress >= 80
                    ? "bg-red-500/20 text-red-400 border-red-500/30 border-none pointer-events-none "
                    : overallDrawdownProgress >= 50
                      ? "bg-amber-500/20 text-amber-400 border-amber-500/30 border-none pointer-events-none"
                      : "bg-emerald-500/20 text-emerald-400 border-emerald-500/30 pointer-events-none"
              }
            >
              {overallDrawdown >= maxOverallDrawdown
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <span className="text-slate-300 text-sm">
                {t("dashboard.rulesPanel.minTradingDays")}
              </span>
              {tradingDays >= minTradingDays ? (
                <CheckCircle className="w-4 h-4 text-emerald-400" />
              ) : (
      ...
      <div>
        <h4 className="text-sm font-medium text-slate-400 mb-3">
          {t("dashboard.rulesPanel.tradingStyleRules")}
        </h4>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {[
            {
              name: t("dashboard.rulesPanel.newsTrading"),
              allowed: challenge?.newsTradingAllowed !== false,
              icon: Newspaper,
            },
            {
              name: t("dashboard.rulesPanel.weekendHolding"),
              allowed: challenge?.weekendHoldingAllowed !== false,
              icon: Calendar,
            },
            {
              name: t("dashboard.rulesPanel.expertAdvisors"),
              allowed: challenge?.eaAllowed !== false,
              icon: Bot,
            },
            {
            ...
            >
              <rule.icon
                className={`w-4 h-4 ${rule.allowed ? "text-emerald-400" : "text-red-400"}`}
              />
              <span
                className={`text-xs ${rule.allowed ? "text-emerald-400" : "text-red-400"}`}
              >
                {rule.name}
              </span>
              {rule.allowed ? (
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            <p className="text-xs text-slate-400">
              {t("dashboard.rulesPanel.leverage")}
            </p>
            <p className="text-white font-semibold">
              1:{challenge?.leverage || account?.leverage || 100}
            </p>
          </div>
          <div>
            <p className="text-xs text-slate-400">
              {t("dashboard.rulesPanel.maxLot", {
                max: t("dashboard.rulesPanel.max"),
                lot: t("dashboard.rulesPanel.lot"),
              })}
            </p>
            <p className="text-white font-semibold">
              {(
          ...
          <div>
            <p className="text-xs text-slate-400">
              {t("dashboard.rulesPanel.profitSplit")}
            </p>
