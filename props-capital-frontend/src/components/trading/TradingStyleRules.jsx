...
const TradingStyleRules = ({ challenge }) => {
  ...
  const tradingStyleRules = [
    { id: 'news', label: t('tradingStyleRules.newsTrading'), icon: Newspaper, allowed: true },
    { id: 'weekend', label: t('tradingStyleRules.weekendHolding'), icon: Calendar, allowed: true },
    { id: 'ea', label: t('tradingStyleRules.expertAdvisors'), icon: Bot, allowed: true },
    { id: 'hedging', label: t('tradingStyleRules.hedging'), icon: GitBranch, allowed: true },
    { id: 'copy', label: t('tradingStyleRules.copyTrading'), icon: Copy, allowed: false },
  ];

  return (
    <div className={cardClass + ' p-4'}>
      <h3 className={`font-bold mb-4 ${textClass}`}>{t('tradingStyleRules.title')}</h3>
          ...
          >
            ...
            <div>
              ...
              <span className={`block text-xs ${rule.allowed ? 'text-emerald-500' : 'text-red-500'}`}>
                {rule.allowed ? t('tradingStyleRules.allowed') : t('tradingStyleRules.notAllowed')}
              </span>
            </div>
          </div>
        ))}
