    ...
    >
      ...
      <div className={`mb-6 rounded-2xl p-5 ${isDark ? 'bg-[#0a0d12]' : 'bg-slate-50'}`}>
        ...
        <p className={`text-sm mt-3 ${isValid ? (isDark ? 'text-gray-300' : 'text-slate-600') : 'text-red-400'}`}>
          {isValid ? (
            <>
              For <span className="text-amber-500 font-bold">{formatFee(customPriceNumber)}</span> you get a{' '}
              <span className="text-amber-500 font-bold">{formatAmount(customAccountSize)}</span> account.
            </>
          ) : (
