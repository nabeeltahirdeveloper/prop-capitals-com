 ...
 */
export function useTradingWebSocket({ accountId, onPositionClosed, onAccountStatusChange }) {
  ...
  useEffect(() => {
    ...
    if (subscribedAccountRef.current) {
      console.log('[WebSocket] Unsubscribing from account:', subscribedAccountRef.current);
      socketRef.current.emit('unsubscribe:account', { accountId: subscribedAccountRef.current });
    }
