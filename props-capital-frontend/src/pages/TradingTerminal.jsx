...
export default function TradingTerminal() {
  ...
  useEffect(() => {
    ...
    console.log(
      "📊 Starting trading days polling for account:",
      selectedAccountId,
    );
    ...
    const updateTradingDays = async () => {
      try {
        ...
        if (summary?.metrics?.tradingDaysCompleted !== undefined) {
          console.log(
            "✅ Updated trading days:",
            summary.metrics.tradingDaysCompleted,
          );
   ...
   */
  const refetchTradesUntilSettled = useCallback(
    async (accountId, beforeMarker) => {
      ...
      devLog(
        "🔄 Starting trade polling for account:",
        accountId,
        "pollId:",
        localPollId,
      );
      ...
      for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        if (beforeMarker) {
          ...
          if (currentTrades.length > beforeMarker.tradeCount) {
            devLog(
              "✅ New trades detected! Count:",
              currentTrades.length,
              "vs",
              beforeMarker.tradeCount,
            );
            return true;
          }
          ...
          if (currentTrades.length > 0 && beforeMarker.latestTradeTimestamp) {
            ...
            const latestCloseTime =
              latestTrade.closedAt || latestTrade.closeTime;
            if (
              latestCloseTime &&
          ...
          const closedTrades = currentTrades.filter(
            (t) => t.closedAt || t.closePrice,
          );
          if (closedTrades.length > (beforeMarker.closedCount || 0)) {
            devLog(
              "✅ New closed trades detected! Closed count:",
              closedTrades.length,
              "vs",
              beforeMarker.closedCount,
            );
            return true;
          }
        } else {
   ...
   */
  const computeBreachSnapshotOverallDD = useCallback(
    (evaluation, currentEquity) => {
      // Priority a: evaluation.drawdownPercent (if present and > 0)
      if (
        evaluation?.drawdownPercent !== undefined &&
        evaluation.drawdownPercent > 0
      ) {
   ...
   */
  const persistViolationValues = useCallback(
    (accountId, evaluation, backendStatus = null) => {
      if (!accountId || typeof window === "undefined") return;

      // Store daily drawdown if daily violation occurred
      if (
        evaluation?.dailyViolated &&
        evaluation?.dailyLossPercent !== undefined &&
        evaluation.dailyLossPercent > 0
      ) {
        sessionStorage.setItem(
          `violation:${accountId}:daily_dd`,
          evaluation.dailyLossPercent.toString(),
        );
        devLog(
          "💾 Stored daily drawdown violation value:",
          evaluation.dailyLossPercent,
        );
      }

      // Store overall drawdown if overall violation occurred
      if (
        evaluation?.drawdownViolated &&
        evaluation?.drawdownPercent !== undefined &&
        evaluation.drawdownPercent > 0
      ) {
        sessionStorage.setItem(
          `violation:${accountId}:overall_dd`,
          evaluation.drawdownPercent.toString(),
        );
        devLog(
          "💾 Stored overall drawdown violation value:",
      ...
      if (backendStatus) {
        ...
        if (
          ...
          devLog(
            "💾 Stored daily drawdown from backend status:",
            evaluation.dailyLossPercent,
          );
        }
        if (
          ...
          devLog(
            "💾 Stored overall drawdown from backend status:",
            evaluation.drawdownPercent,
          );
        }
      }
  ...
  useEffect(() => {
    if (!selectedAccountId || !backendTrades || !Array.isArray(backendTrades))
      return;

    try {
      // Find all auto-closed trades with breach snapshots
      const autoClosedTrades = backendTrades.filter(
        (t) =>
          t.breachTriggered &&
          t.closePrice !== null &&
      ...
      if (autoClosedTrades.length > 0) {
        ...
        const mostRecentBreachTrade = autoClosedTrades.sort((a, b) => {
          const aTime = a.breachAt
            ? new Date(a.breachAt).getTime()
            : a.closedAt
              ? new Date(a.closedAt).getTime()
              : 0;
          const bTime = b.breachAt
            ? new Date(b.breachAt).getTime()
            : b.closedAt
        ...
        if (typeof window !== "undefined") {
          if (
            mostRecentBreachTrade.breachDrawdownPercentDaily !== null &&
            ...
            sessionStorage.setItem(
              `violation:${selectedAccountId}:daily_dd`,
              mostRecentBreachTrade.breachDrawdownPercentDaily.toString(),
            );
            devLog(
              "💾 [TRADE BREACH] Extracted daily drawdown from trade breach snapshot:",
              mostRecentBreachTrade.breachDrawdownPercentDaily,
            );
          }

          if (
            mostRecentBreachTrade.breachDrawdownPercentOverall !== null &&
            ...
            sessionStorage.setItem(
              ...
              mostRecentBreachTrade.breachDrawdownPercentOverall.toString(),
            );
            sessionStorage.setItem(
            ...
            devLog(
              "💾 [TRADE BREACH] Extracted overall drawdown from trade breach snapshot:",
              mostRecentBreachTrade.breachDrawdownPercentOverall,
            );
          }
        }
      }
  ...
  const { data: backendPendingOrders, isLoading: isLoadingPendingOrders } =
    useQuery({
      ...
      queryFn: async () => {
        ...
        try {
          ...
          devLog(
            "📥 Fetched pending orders from backend for account:",
            selectedAccountId,
            "Count:",
            orders?.length || 0,
          );
      ...
      enabled:
        !!selectedAccountId && hasValidAccount && !selectedAccount?.isDemo,
      refetchOnWindowFocus: false,
      retry: 2,
      retryDelay: 1000,
      staleTime: 0,
  ...
  const { isConnected: wsConnected, connectionStatus: wsConnectionStatus } =
    useTradingWebSocket({
      ...
      onPositionClosed: useCallback(
        (event) => {
          ...
          setPositions((prev) => {
            ...
            devLog(
              "⚡ [WebSocket] Removed position from state:",
              event.tradeId,
              "Remaining:",
              filtered.length,
            );
          ...
          if (selectedAccountId) {
            syncAccountFromBackend(selectedAccountId, false, false).catch(
              (err) => {
                devWarn(
                  "Failed to sync account after WebSocket position close:",
                  err,
                );
              },
            );
      ...
      onAccountStatusChange: useCallback(
        (event) => {
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        const backendTrades = queryClient.getQueryData([
          "trades",
          selectedAccountId,
        ]);
        ...
        if (closedPositionIds.length > 0) {
          devLog(
            "🔄 [Fallback Polling] Detected",
            closedPositionIds.length,
            "closed positions",
          );
  ...
  useEffect(() => {
    ...
    // Clear dismissed modal state for this account so violation modal shows again when opening
  ...
  useEffect(() => {
    ...
    if (backendTrades === undefined) {
      devLog(
        "⏳ Trades data not available yet for account:",
        selectedAccountId,
      );
      return;
    }
    ...
    devLog(
      "🔄 Processing trades from backend for account:",
      selectedAccountId,
      "Total trades:",
      tradesArray.length,
    );
    ...
    devLog(
      `✅ Loaded ${openTrades.length} open positions and ${closedTrades.length} closed trades`,
    );
    ...
    if (openTrades.length === 0 && closedTrades.length === 0) {
      devLog(
        "⚠️ No trades found in backend for account:",
        selectedAccountId,
        "- clearing positions",
      );
      setPositions([]);
  ...
  useEffect(() => {
    ...
    if (isLoadingPendingOrders) {
      devLog(
        "⏳ Waiting for pending orders to load for account:",
        selectedAccountId,
      );
      return;
    }
    ...
    const ordersArray = Array.isArray(backendPendingOrders)
      ? backendPendingOrders
      : [];
    devLog(
      "🔄 Processing pending orders from backend for account:",
      selectedAccountId,
      "Total orders:",
      ordersArray.length,
    );
          ...
          processPriceTick(
            ...
            .then((response) => {
              if (response.statusChanged) {
                ...
                const snapshotOverallDD = computeBreachSnapshotOverallDD(
                  { drawdownPercent: response.violationOverallDrawdown },
                  currentEquity,
                );
                if (
                  snapshotOverallDD !== null &&
                  snapshotOverallDD > 0 &&
                  typeof window !== "undefined"
                ) {
                  sessionStorage.setItem(
                    `violation:${selectedAccountId}:snapshot_overall_dd`,
                    snapshotOverallDD.toString(),
                  );
                  devLog(
                    "💾 [ATOMIC SNAPSHOT] [Price-Tick] Stored overall drawdown at breach moment:",
                    snapshotOverallDD,
                  );
                }
                ...
                if (response.positionsClosed > 0) {
                  const beforeTrades =
                    ...
                    [];
                  const beforeMarker = {
                    tradeCount: Array.isArray(beforeTrades)
                      ? beforeTrades.length
                      : 0,
                  ...
                  queryClient.invalidateQueries({
                    ...
                    predicate: (query) => {
                      ...
                      return (
                        queryKey.includes(selectedAccountId) ||
                        (Array.isArray(queryKey[1]) &&
                          queryKey[1].includes(selectedAccountId))
                      );
                    },
                  });

                  refetchTradesUntilSettled(
                    selectedAccountId,
                    beforeMarker,
                  ).catch((err) => {
                ...
                if (response.violationType) {
                  if (response.violationDailyDrawdown !== undefined) {
                    sessionStorage.setItem(
                      `violation:${selectedAccountId}:daily_dd`,
                      response.violationDailyDrawdown.toString(),
                    );
                  }
                  if (response.violationOverallDrawdown !== undefined) {
                    sessionStorage.setItem(
                      `violation:${selectedAccountId}:overall_dd`,
                      response.violationOverallDrawdown.toString(),
                    );
                  }
  ...
  useEffect(() => {
    ...
    pendingOrders.forEach((order) => {
      ...
      const prevPrice =
        prevPriceData && typeof prevPriceData === "object"
          ? prevPriceData.bid
          : prevPriceData;
    ...
    if (ordersToExecute.length > 0) {
      ...
      console.log(
        `🔄 Executing ${ordersToExecute.length} pending limit orders...`,
      );
          ...
          try {
            ...
            if (order.isDemo || selectedAccount?.isDemo) {
              ...
              setPositions((prev) => {
                ...
                // Persist demo data
                const newPendingOrders = demoPendingOrders.filter(
                  (o) => o.id !== order.id,
                );
            ...
            const response = await executePendingOrder(
              order.id,
              order.limitPrice,
            );
            ...
            if (response?.trade?.id) {
              ...
              console.log(
                `✅ Limit order executed via backend API: ${response.trade.id}`,
              );
            }
          } catch (error) {
  ...
  const scheduleAccountUpdate = useCallback(() => {
    ...
    rafIdRef.current = requestAnimationFrame(() => {
      ...
      if (
        pending.equity !== null &&
        Number.isFinite(pending.equity) &&
        Math.abs((prevAccount.equity || 0) - pending.equity) >= 1.0
      ) {
        updates.equity = pending.equity;
      }
      if (
        pending.floatingPnL !== null &&
        Number.isFinite(pending.floatingPnL) &&
        Math.abs((prevAccount.floatingPnL || 0) - pending.floatingPnL) >= 1.0
      ) {
      ...
      if (
        pending.profitPercent !== null &&
        Number.isFinite(pending.profitPercent)
      ) {
        ...
        if (
          prevProfit === 0 ||
          Math.abs(prevProfit - pending.profitPercent) >= 0.01
        ) {
      ...
      if (
        pending.realizedProfitPercent !== null &&
        Number.isFinite(pending.realizedProfitPercent)
      ) {
        updates.realizedProfitPercent = pending.realizedProfitPercent;
      }
      if (
        pending.liveProfitPercent !== null &&
        Number.isFinite(pending.liveProfitPercent)
      ) {
        updates.liveProfitPercent = pending.liveProfitPercent;
      }
      if (
        pending.profitForTarget !== null &&
        Number.isFinite(pending.profitForTarget)
      ) {
      ...
      if (
        ...
        Number.isFinite(pending.overallDrawdown)
      ) {
      ...
      if (
        pending.dailyDrawdown !== null &&
        Number.isFinite(pending.dailyDrawdown)
      ) {
      ...
      if (
        pending.highestBalance !== null &&
        Number.isFinite(pending.highestBalance)
      ) {
        updates.highestBalance = pending.highestBalance;
      }
      if (
        pending.maxEquityToDate !== null &&
        Number.isFinite(pending.maxEquityToDate)
      ) {
  ...
  useEffect(() => {
    const updateMetrics = () => {
      ...
      positions.forEach((pos) => {
        ...
        if (process.env.NODE_ENV !== "production") {
          console.log(
            `[Equity Debug] Position: ${pos.symbol} ${pos.type.toUpperCase()}`,
          );
          console.log(`  Entry Price: ${pos.entryPrice}`);
          console.log(
            `  Current Price (${pos.type === "buy" ? "BID" : "ASK"}): ${currentPrice}`,
          );
  ...
  // Track dismissed modals per account (user explicitly closed)
  ...
  useEffect(() => {
    if (!selectedAccountId) return; // Don't run if no account selected

    const statusUpper = String(account.status || "").toUpperCase();
    const wasDismissed =
      dismissedModalAccountsRef.current.has(selectedAccountId);

    console.log("[ViolationModal Debug] Full account object:", account);
    console.log(
      "[ViolationModal Debug] Status:",
      account.status,
      "StatusUpper:",
      statusUpper,
      "AccountId:",
      ...
      "WasDismissed:",
      wasDismissed,
      "violationModal state:",
      violationModal,
    );

    // Check for daily lock status (includes 'DAILY')
    if (statusUpper.includes("DAILY")) {
      console.log("[ViolationModal] Daily lock detected!");
      setAccountStatusBanner({
        type: "warning",
        title: t("terminal.dailyLocked") || "Daily Loss Limit Reached",
        message:
          t("terminal.dailyLockedMessage") ||
          "Your account has been locked until tomorrow due to exceeding the daily loss limit.",
      });
    ...
    // Check for disqualification/failure status (includes 'FAIL' or 'DISQUAL')
      ...
      console.log("[ViolationModal] Disqualification detected!");
      setAccountStatusBanner({
        type: "error",
        title: t("terminal.disqualified") || "Challenge Disqualified",
        message:
          t("terminal.disqualifiedMessage") ||
          "Your account has been disqualified due to exceeding the maximum drawdown limit.",
      });
      // Show modal if not dismissed by user for this account
      if (!wasDismissed) {
      ...
      // Clear modal when switching to active account (no violation)
      setViolationModal(null);
      // Clear dismissed state when switching to active account
      dismissedModalAccountsRef.current.delete(selectedAccountId);
    }
  }, [account.status, selectedAccountId, t]);

  // Handler for when user closes the violation modal
  const handleViolationModalClose = useCallback(() => {
    if (selectedAccountId) {
      dismissedModalAccountsRef.current.add(selectedAccountId);
    }
  ...
  const handleSaveModifiedPosition = useCallback(
    async (updatedPosition) => {
      ...
      try {
        console.log(
          "💾 Modifying position via backend API:",
          updatedPosition.id,
        );
        ...
        const response = await modifyPosition(
          updatedPosition.id,
          modificationData,
        );
        ...
        if (response?.trade) {
          ...
          setPositions((prev) =>
            prev.map((p) => (p.id === updatedPosition.id ? updatedPos : p)),
          );
              ...
              {sidebarCollapsed ? (
                ...
                >
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center gap-3">
                    ...
                    <span
                      className="text-slate-500 text-[10px] font-medium tracking-wider uppercase group-hover:text-slate-300 transition-colors"
                      style={{
                  ...
                  {/* Bottom - Current symbol indicator */}
