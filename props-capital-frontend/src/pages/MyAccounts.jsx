...
export default function MyAccounts() {
  ...
  const filteredAccounts = filter === 'all'
    ...
    : displayAccounts.filter(a => {
      if (filter === 'challenges') return a.status === 'active' && (a.current_phase === 'phase1' || a.current_phase === 'phase2');
      if (filter === 'phase1') return a.current_phase === 'phase1' && a.status === 'active';
      if (filter === 'phase2') return a.current_phase === 'phase2' && a.status === 'active';
      if (filter === 'funded') return a.status === 'funded' || a.current_phase === 'funded';
      if (filter === 'failed') return a.status === 'failed' || a.current_phase === 'failed';
      return true;
    });

  const accountCounts = {
    all: displayAccounts.length,
    challenges: displayAccounts.filter(a => a.status === 'active').length,
    phase1: displayAccounts.filter(a => a.current_phase === 'phase1' && a.status === 'active').length,
    phase2: displayAccounts.filter(a => a.current_phase === 'phase2' && a.status === 'active').length,
    funded: displayAccounts.filter(a => a.status === 'funded').length,
    failed: displayAccounts.filter(a => a.status === 'failed').length,
  };
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-white">{t('myAccounts.title')}</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4">
        {[
          { key: 'challenges', label: t('myAccounts.activeChallenges'), icon: Activity, color: 'emerald' },
          { key: 'phase1', label: t('myAccounts.phase1'), icon: Target, color: 'blue' },
          { key: 'phase2', label: t('myAccounts.phase2'), icon: Target, color: 'purple' },
          { key: 'funded', label: t('myAccounts.funded'), icon: Award, color: 'amber' },
          { key: 'failed', label: t('myAccounts.failed'), icon: AlertTriangle, color: 'red' },
        ].map((cat) => (
          <Card
            key={cat.key}
          ...
          >
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 bg-${cat.color}-500/20 rounded-lg flex items-center justify-center`}>
                <cat.icon className={`w-5 h-5 text-${cat.color}-400`} />
              </div>
              <div>
                <p className="text-lg sm:text-2xl font-bold text-white">{accountCounts[cat.key]}</p>
      ...
      {filteredAccounts.length === 0 ? (
        <Card className="bg-slate-900 border-slate-800 p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-white mb-2">{t('myAccounts.noAccounts')}</h3>
          <p className="text-slate-400 mb-6">{t('myAccounts.noAccountsDesc')}</p>
          <Link to={createPageUrl('TraderBuyChallenge')}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {filteredAccounts.map((account) => {
            ...
            const profitProgress = account.status === 'funded' ? 100 : Math.max(0, Math.min((account.current_profit_percent / profitTarget) * 100, 100));
            const dailyDDUsed = (account.daily_drawdown_percent / 5) * 100;
            const overallDDUsed = (account.overall_drawdown_percent / 10) * 100;

            return (
              <Card key={account.id} className={`bg-slate-900 border-slate-800 overflow-hidden ${account.status === 'funded' ? 'ring-1 ring-amber-500/50' : ''} ${account.status === 'failed' ? 'opacity-75' : ''}`}>
                {/* Header */}
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-sm text-slate-400">
                        ...
                        >
                          {copiedId === account.id ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                        </button>
                      </div>
                    </div>
                ...
                <div className="p-5">
                  <div className="flex items-center gap-4 mb-4">
                    <ProgressRing
                      ...
                      size={80}
                      strokeWidth={6}
                      color={account.status === 'funded' ? '#f59e0b' : account.status === 'failed' ? '#ef4444' : profitProgress >= 100 ? '#10b981' : '#06b6d4'}
                    />
                    <div className="flex-1">
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-slate-400">{t('myAccounts.balance')}</span>
                        <span className="text-white font-medium">${account.current_balance?.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-slate-400">{t('myAccounts.profit')}</span>
                        <span className={`font-medium ${account.current_profit_percent >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                          {account.current_profit_percent >= 0 ? '+' : ''}{account.current_profit_percent?.toFixed(2)}%
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-400">{t('myAccounts.days')}</span>
                        <span className="text-white">{account.trading_days_count}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-slate-800/50 rounded-lg p-3">
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-slate-400">{t('myAccounts.dailyDD')}</span>
                        <span className={dailyDDUsed > 80 ? 'text-amber-400' : 'text-white'}>
                    ...
                    <div className="bg-slate-800/50 rounded-lg p-3">
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-slate-400">{t('myAccounts.overallDD')}</span>
                        <span className={overallDDUsed > 80 ? 'text-red-400' : 'text-white'}>
                  ...
                  {/* Violation Message */}
                  {account.status === 'failed' && account.violation_reason && (
                    <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 mb-4">
                      <div className="flex items-center gap-2">
                        <AlertTriangle className="w-4 h-4 text-red-400" />
                        <p className="text-sm text-red-400">{translateViolationMessage(account.violation_reason, t)}</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Link to={`${createPageUrl('ChallengeProgress')}?id=${account.id}`} className="flex-1">
                      ...
                      <Link to={`${createPageUrl('TradeHistory')}?account=${account.id}`} className="flex-1">
                        <Button variant="ghost" size="sm" className="w-full text-slate-400 hover:text-white hover:bg-slate-800">
      ...
      <Card className="bg-gradient-to-r from-cyan-500/10 to-purple-500/10 border-cyan-500/30 p-4 md:p-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-white mb-1">{t('myAccounts.needChallenge')}</h3>
            <p className="text-slate-400 text-sm">{t('myAccounts.needChallengeDesc')}</p>
          </div>
          <Link to={createPageUrl('TraderBuyChallenge')}>
            <Button className="bg-gradient-to-r from-emerald-500 to-cyan-500">
