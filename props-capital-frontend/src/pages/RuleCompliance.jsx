...
import { getAccountById, getUserAccounts, getAccountRules, getPhaseTransitions, getDailyComplianceHistory } from '@/api/accounts';
...
export default function RuleCompliance() {
    ...
    return (
      <div className="space-y-6">
        ...
        <Card className="bg-slate-900 border-slate-800 p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-white mb-2">{t('ruleCompliance.noAccounts')}</h3>
          <p className="text-slate-400 mb-6">{t('ruleCompliance.noAccountsDesc') || 'Purchase a challenge to view rule compliance and trading rules.'}</p>
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-slate-400">{t('ruleCompliance.loadingCompliance')}</div>
      </div>
    );
  }
  ...
  const dailyDrawdownPercent = metrics.dailyDrawdownPercent || account.dailyDrawdownPercent || 0;
  const overallDrawdownPercent = metrics.overallDrawdownPercent || account.overallDrawdownPercent || 0;
  const tradingDaysCount = metrics.tradingDaysCompleted || account.tradingDaysCount || 0;

  const phase1Target = challengeRules.phase1TargetPercent || challenge.phase1TargetPercent || 8;
  const phase2Target = challengeRules.phase2TargetPercent || challenge.phase2TargetPercent || 5;
  const maxDailyDD = challengeRules.dailyDrawdownPercent || challenge.dailyDrawdownPercent || 5;
  const maxOverallDD = challengeRules.overallDrawdownPercent || challenge.overallDrawdownPercent || 10;
  const minTradingDays = challengeRules.minTradingDays || challenge.minTradingDays || 5;

  // Determine current phase target
  const currentTarget = currentPhase === 'phase2' || currentPhase === 'funded'
    ? phase2Target
    : phase1Target;
  ...
  const daysElapsed = account.createdAt
    ? Math.floor((new Date().getTime() - new Date(account.createdAt).getTime()) / (1000 * 60 * 60 * 24))
    : 0;

  // Convert compliance history to rule history format
  const ruleHistory = complianceHistory.flatMap((day) => {
    const history = [];

    // Daily drawdown entry
    const dailyStatus = day.dailyDrawdown <= maxDailyDD
      ? (day.dailyDrawdown >= maxDailyDD * 0.8 ? 'warning' : 'passed')
      : 'failed';
    history.push({
      date: day.date,
    ...
    const overallStatus = day.overallDrawdown <= maxOverallDD
      ? (day.overallDrawdown >= maxOverallDD * 0.8 ? 'warning' : 'passed')
      : 'failed';
    history.push({
      date: day.date,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <Select value={accountId || ''} onValueChange={(value) => {
          setAccountId(value);
          window.history.replaceState({}, '', `${window.location.pathname}?account=${value}`);
        }}>
      ...
      <Card className="bg-gradient-to-r from-emerald-500/10 to-cyan-500/10 border-emerald-500/30 p-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 sm:w-16 sm:h-16 bg-emerald-500/20 rounded-full flex items-center justify-center flex-shrink-0">
              <CheckCircle className="w-6 h-6 sm:w-8 sm:h-8 text-emerald-400" />
            </div>
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-white">{t('ruleCompliance.allRulesCompliant')}</h2>
              <p className="text-sm sm:text-base text-slate-400">{t('ruleCompliance.goodStanding')}</p>
            </div>
          </div>
          <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 text-base sm:text-lg px-3 py-1 sm:px-4 sm:py-2">
      ...
      <ChallengeRulesPanel
        account={{
          ...
          equity: account.current_equity || account.current_balance,
          profitPercent: account.current_profit_percent || 0,
          dailyDrawdown: account.daily_drawdown_percent || 0,
          overallDrawdown: account.overall_drawdown_percent || 0,
          tradingDays: tradingDaysCount,
          daysElapsed: daysElapsed,
          phase: currentPhase,
          startingBalance: account.initial_balance,
          highestBalance: account.highest_balance || account.initial_balance,
          profitTarget: challenge?.phase1_profit_target || 10,
          maxDailyDrawdown: challenge?.max_daily_drawdown || 5,
          maxOverallDrawdown: challenge?.max_overall_drawdown || 10,
      ...
      <Card className="bg-slate-900 border-slate-800 p-6">
        <h3 className="text-lg font-semibold text-white mb-4">{t('ruleCompliance.tradingStyleRules')}</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
          {[
            ...
            <div
              key={i}
              className={`p-4 rounded-xl border ${rule.allowed
                ? 'bg-emerald-500/5 border-emerald-500/20'
                : 'bg-red-500/5 border-red-500/20'
                }`}
            >
              <div className="flex items-start gap-3">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${rule.allowed ? 'bg-emerald-500/20' : 'bg-red-500/20'
                  }`}>
                  <rule.icon className={`w-5 h-5 ${rule.allowed ? 'text-emerald-400' : 'text-red-400'}`} />
                </div>
      ...
      <Card className="bg-slate-900 border-slate-800 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-white">{t('ruleCompliance.complianceHistory')}</h3>
        ...
        <div className="space-y-2">
          {isLoadingHistory ? (
            ...
            <div className="text-center text-slate-400 py-8">
              ...
              <p className="text-sm font-medium text-white mb-1">{t('ruleCompliance.noComplianceHistory')}</p>
              <p className="text-xs">{t('ruleCompliance.noComplianceHistoryDesc')}</p>
            </div>
          ) : (
            ruleHistory.map((entry, i) => (
              <div
                key={i}
                className={`flex items-center justify-between p-3 rounded-lg ${entry.status === 'passed' ? 'bg-slate-800/50' :
                  entry.status === 'warning' ? 'bg-amber-500/10 border border-amber-500/30' :
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-white text-sm font-medium">{entry.rule}</p>
                    <p className="text-xs text-slate-400">{entry.date}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className={`font-mono text-sm ${entry.status === 'passed' ? 'text-emerald-400' :
                    entry.status === 'warning' ? 'text-amber-400' : 'text-red-400'
                    }`}>
                    {entry.value}
                  </p>
                  <p className="text-xs text-slate-500">{t('ruleCompliance.limit')}: {entry.limit}</p>
                </div>
              </div>
            ))
          )}
      ...
      <Card className="bg-slate-800/50 border-slate-700 p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="text-white font-medium mb-2">{t('ruleCompliance.importantInformation')}</h4>
