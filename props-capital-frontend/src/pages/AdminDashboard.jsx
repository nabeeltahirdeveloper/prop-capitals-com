...
import { adminGetDashboardOverview, adminGetRecentAccounts, adminGetRecentViolations, adminGetRevenueChart, adminGetRegistrationsChart } from '@/api/admin';
...
export default function AdminDashboard() {
  ...
  const revenueData = revenueChartData.length > 0
  ...
  const recentViolations = (recentViolationsData || []).map(violation => {
    ...
    return {
      id: violation.id,
      type: violationType,
      description: violation.message || t(`admin.dashboard.violations.${violationType}.description`, { defaultValue: `${violation.type} violation` }),
      threshold: threshold,
  ...
  const accountColumns = [
    { header: t('admin.dashboard.table.account'), accessorKey: 'account_number', cell: (row) => <span className="font-medium text-white">{row.account_number}</span> },
    { header: t('admin.dashboard.table.platform'), accessorKey: 'platform' },
    { header: t('admin.dashboard.table.phase'), accessorKey: 'current_phase', cell: (row) => <StatusBadge status={row.current_phase} /> },
    { header: t('admin.dashboard.table.status'), accessorKey: 'status', cell: (row) => <StatusBadge status={row.status} /> },
    {
      header: t('admin.dashboard.table.created'), accessorKey: 'created_date', cell: (row) => {
        try {
  ...
  const violationColumns = [
    { header: t('admin.dashboard.table.type'), accessorKey: 'type', cell: (row) => <span className="text-red-400 capitalize">{t(`admin.dashboard.violations.${row.type}.type`, { defaultValue: row.type?.replace(/_/g, ' ') })}</span> },
    {
      header: t('admin.dashboard.table.description'), accessorKey: 'description', cell: (row) => {
    ...
    {
      header: t('admin.dashboard.table.time'), accessorKey: 'created_date', cell: (row) => {
        try {
          if (!row.created_date) return '-';
          const date = new Date(row.created_date);
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-white">{t('admin.dashboard.title')}</h1>
        <p className="text-sm sm:text-base text-slate-400">{t('admin.dashboard.subtitle')}</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <StatsCard
          ...
          change={overview.usersChangePercent !== undefined ? t('admin.dashboard.stats.changeThisMonth', { percent: Math.round(overview.usersChangePercent) }) : undefined}
          changeType={overview.usersChangePercent >= 0 ? 'positive' : 'negative'}
        />
        <StatsCard
        ...
        <StatsCard
          ...
          change={overview.revenueChangePercent !== undefined ? t('admin.dashboard.stats.changeThisMonth', { percent: Math.round(overview.revenueChangePercent) }) : undefined}
          changeType={overview.revenueChangePercent >= 0 ? 'positive' : 'negative'}
        />
        <StatsCard
      ...
      <Card className="bg-slate-900 border-slate-800 p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 sm:mb-6">
          <h3 className="text-base sm:text-lg font-semibold text-white">{t('admin.dashboard.chart.title')}</h3>
          <div className="flex items-center gap-3 sm:gap-4 text-xs sm:text-sm">
            <div className="flex items-center gap-2">
              <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-emerald-500" />
              <span className="text-slate-400">{t('admin.dashboard.chart.revenue')}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-purple-500" />
              <span className="text-slate-400">{t('admin.dashboard.chart.payouts')}</span>
            </div>
          </div>
        </div>
        <div className="h-[200px] sm:h-[250px] md:h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={revenueData}>
              ...
              <XAxis dataKey="date" stroke="#64748b" tick={{ fill: '#64748b', fontSize: 10 }} interval="preserveStartEnd" />
              <YAxis stroke="#64748b" tick={{ fill: '#64748b', fontSize: 10 }} tickFormatter={(v) => `$${v / 1000}k`} width={45} />
              <Tooltip
                contentStyle={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '8px' }}
                labelStyle={{ color: '#fff' }}
                labelFormatter={(label) => `${t('admin.dashboard.chart.tooltip.date')}: ${label}`}
              ...
              <Area type="monotone" dataKey="payouts" stroke="#a855f7" fill="url(#payoutGradient)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <Card className="bg-slate-900 border-slate-800 p-4 sm:p-6">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <h3 className="text-base sm:text-lg font-semibold text-white">{t('admin.dashboard.recentAccounts.title')}</h3>
            <Link to={createPageUrl('AdminAccounts')}>
              <Button variant="ghost" size="sm" className="text-slate-400 hover:text-white text-xs sm:text-sm px-2 sm:px-3">
                {t('admin.dashboard.viewAll')} <ArrowRight className="w-3 h-3 sm:w-4 sm:h-4 ml-1" />
              </Button>
            </Link>
          </div>
          <DataTable columns={accountColumns} data={displayRecentAccounts} emptyMessage={t('admin.dashboard.noDataFound')} />
        </Card>
        ...
        <Card className="bg-slate-900 border-slate-800 p-4 sm:p-6">
          ...
          <DataTable columns={violationColumns} data={displayRecentViolations} emptyMessage={t('admin.dashboard.noDataFound')} />
        </Card>
      </div>
        ...
        <Link to={createPageUrl('AdminUsers')}>
          <Card className="bg-slate-900 border-slate-800 p-3 sm:p-4 hover:border-emerald-500/50 transition-colors cursor-pointer h-full">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <span className="text-white font-medium text-xs sm:text-sm">{t('admin.dashboard.quickActions.manageUsers')}</span>
            </div>
          </Card>
        </Link>
        <Link to={createPageUrl('AdminChallenges')}>
          <Card className="bg-slate-900 border-slate-800 p-3 sm:p-4 hover:border-cyan-500/50 transition-colors cursor-pointer h-full">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <span className="text-white font-medium text-xs sm:text-sm">{t('admin.dashboard.quickActions.manageChallenges')}</span>
            </div>
          </Card>
        </Link>
        <Link to={createPageUrl('AdminPayouts')}>
          <Card className="bg-slate-900 border-slate-800 p-3 sm:p-4 hover:border-purple-500/50 transition-colors cursor-pointer h-full">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <span className="text-white font-medium text-xs sm:text-sm">{t('admin.dashboard.quickActions.reviewPayouts')}</span>
            </div>
          </Card>
        </Link>
        <Link to={createPageUrl('AdminSettings')}>
          <Card className="bg-slate-900 border-slate-800 p-3 sm:p-4 hover:border-amber-500/50 transition-colors cursor-pointer h-full">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <span className="text-white font-medium text-xs sm:text-sm">{t('admin.dashboard.quickActions.platformSettings')}</span>
            </div>
          </Card>
        </Link>
      </div>
