...
import { getUserAccounts, getAccountById, getAccountSummary, getAccountRules, getAnalytics } from '@/api/accounts';
...
export default function AccountDetails() {
    ...
    return (
      <div className="space-y-6">
        ...
        <Card className="bg-slate-900 border-slate-800 p-12 text-center">
          ...
          <p className="text-slate-400 mb-6">{t('accountDetails.noAccountsDesc') || 'Purchase a challenge to view your account details and trading dashboard.'}</p>
  ...
  const challengeRules = summaryData?.challengeRules || rulesData?.challengeRules || challenge;
  ...
  const account = {
    ...
    current_equity: accountData.equity || accountData.balance || accountData.initialBalance || 0,
    ...
    trading_days_count: metrics.tradingDaysCompleted || accountData.tradingDaysCount || 0,
    start_date: accountData.createdAt || new Date().toISOString(),
    profit_target: accountData.phase === 'PHASE2'
    ...
    max_daily_dd: challengeRules.dailyDrawdownPercent || challenge.dailyDrawdownPercent || 5,
    max_overall_dd: challengeRules.overallDrawdownPercent || challenge.overallDrawdownPercent || 10,
    min_trading_days: challengeRules.minTradingDays || challenge.minTradingDays || 4,
    leverage: '1:100',
    news_trading_allowed: challengeRules.newsTradingAllowed ?? challenge.newsTradingAllowed ?? true,
    weekend_holding_allowed: challengeRules.weekendHoldingAllowed ?? challenge.weekendHoldingAllowed ?? true,
  ...
  const trades = (tradesData || []).map(trade => {
    ...
    return {
      ...
      open_time: trade.openedAt ? format(new Date(trade.openedAt), 'yyyy-MM-dd HH:mm') : new Date().toISOString(),
      close_time: trade.closedAt ? format(new Date(trade.closedAt), 'yyyy-MM-dd HH:mm') : null,
      status: trade.closePrice || trade.closedAt ? 'closed' : 'open'
    };
  });

  // Get equity history from summary or analytics
  const equityHistory = summaryData?.equityHistory || accountData.equityShots || analyticsData?.equityCurve || [];
  const equityMetrics = equityHistory.length > 0
  ...
  const dailyPnLHistory = accountData.analytics?.dailyPnL || analyticsData?.dailyPnL || summaryData?.dailyPnL || [];
  const profitMetrics = dailyPnLHistory.length > 0
  ...
  const profitProgress = Math.max(0, Math.min((account.current_profit_percent / account.profit_target) * 100, 100));
  const dailyDDUsed = (account.daily_drawdown_percent / account.max_daily_dd) * 100;
  const overallDDUsed = (account.overall_drawdown_percent / account.max_overall_dd) * 100;

  // Calculate stats from analytics or trades data
  const analyticsStats = analyticsData?.statistics || {};
  const closedTrades = (tradesData || []).filter(t => t.closePrice || t.closedAt);

  // Calculate stats from trades if analytics not available
  const calculatedStats = closedTrades.length > 0 ? (() => {
    ...
    const largestLoss = losingProfits.length > 0 ? Math.min(...losingProfits) : 0;
    ...
    const avgTradeSize = closedTrades.length > 0
      ? closedTrades.reduce((sum, t) => sum + (t.volume || t.lotSize || 0), 0) / closedTrades.length
      : 0;
    ...
    const avgDuration = avgDurationMs > 0 ? `${hours}h ${minutes}m` : '0h 0m';
  ...
  const stats = {
    winRate: integratedStats.winRate ?? analyticsStats.winRate ?? calculatedStats.winRate ?? 0,
    avgRR: integratedStats.avgRR ?? analyticsStats.avgRR ?? calculatedStats.avgRR ?? 0,
    totalTrades: integratedStats.totalTrades ?? analyticsStats.totalTrades ?? calculatedStats.totalTrades ?? 0,
    avgTradeSize: integratedStats.avgTradeSize ?? calculatedStats.avgTradeSize ?? 0,
    largestWin: integratedStats.bestTrade ?? analyticsStats.bestTrade ?? calculatedStats.largestWin ?? 0,
    largestLoss: integratedStats.worstTrade ?? analyticsStats.worstTrade ?? calculatedStats.largestLoss ?? 0,
    profitFactor: integratedStats.profitFactor ?? analyticsStats.profitFactor ?? calculatedStats.profitFactor ?? 0,
    avgDuration: integratedStats.avgDuration ?? calculatedStats.avgDuration ?? '0h 0m'
  };
  ...
  const ruleCompliance = rulesData ? [
    {
      ...
      status: metrics.dailyDrawdownPercent <= challengeRules.dailyDrawdownPercent ? 'ok' : 'violated',
    ...
    },
    {
      ...
      status: metrics.overallDrawdownPercent <= challengeRules.overallDrawdownPercent ? 'ok' : 'violated',
    ...
    },
    {
      ...
      status: metrics.tradingDaysCompleted >= challengeRules.minTradingDays ? 'completed' : 'pending',
      value: `${metrics.tradingDaysCompleted || 0} ${t('accountDetails.days')}`,
      limit: `${challengeRules.minTradingDays || 4} ${t('accountDetails.days')}`
    },
    { rule: t('accountDetails.newsTrading'), status: 'ok', value: t('accountDetails.compliant'), limit: t('accountDetails.allowed') },
    { rule: t('accountDetails.weekendHolding'), status: 'ok', value: t('accountDetails.compliant'), limit: t('accountDetails.allowed') },
  ] : [
    { rule: t('accountDetails.dailyDrawdown'), status: account.daily_drawdown_percent <= account.max_daily_dd ? 'ok' : 'violated', value: `${account.daily_drawdown_percent}%`, limit: `${account.max_daily_dd}%` },
    { rule: t('accountDetails.overallDrawdown'), status: account.overall_drawdown_percent <= account.max_overall_dd ? 'ok' : 'violated', value: `${account.overall_drawdown_percent}%`, limit: `${account.max_overall_dd}%` },
    { rule: t('accountDetails.minTradingDays'), status: account.trading_days_count >= account.min_trading_days ? 'completed' : 'pending', value: `${account.trading_days_count} ${t('accountDetails.days')}`, limit: `${account.min_trading_days} ${t('accountDetails.days')}` },
    { rule: t('accountDetails.newsTrading'), status: 'ok', value: t('accountDetails.compliant'), limit: t('accountDetails.allowed') },
    { rule: t('accountDetails.weekendHolding'), status: 'ok', value: t('accountDetails.compliant'), limit: t('accountDetails.allowed') },
  ];

  const tradeColumns = [
    { header: t('accountDetails.time'), accessorKey: 'open_time', cell: (row) => <span className="text-slate-300 text-sm">{row.open_time}</span> },
    { header: t('accountDetails.symbol'), accessorKey: 'symbol', cell: (row) => <span className="text-white font-medium">{row.symbol}</span> },
    {
      ...
      cell: (row) => (
        <span className={`px-2 py-1 rounded text-xs font-medium ${row.type === 'buy' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
          {row.type === 'buy' ? t('accountDetails.buy').toUpperCase() : t('accountDetails.sell').toUpperCase()}
        </span>
      )
    },
    { header: t('accountDetails.lots'), accessorKey: 'lot_size', cell: (row) => <span className="text-slate-300">{row.lot_size?.toFixed(2)}</span> },
    { header: t('accountDetails.open'), accessorKey: 'open_price', cell: (row) => <span className="text-slate-300">{row.open_price?.toFixed(2)}</span> },
    { header: t('accountDetails.close'), accessorKey: 'close_price', cell: (row) => <span className="text-slate-300">{row.close_price ? row.close_price.toFixed(2) : '-'}</span> },
    {
      header: t('accountDetails.pl'),
      accessorKey: 'profit',
      cell: (row) => (
        <span className={`font-medium ${row.profit >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
    ...
    },
    {
      ...
      accessorKey: 'status',
      cell: (row) => (
        <Badge className={row.status === 'open' ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-500/20 text-slate-400'}>
          {row.status === 'open' ? t('accountDetails.open') : t('accountDetails.closed')}
        </Badge>
      )
    },
  ];
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-4 text-sm text-slate-400 mt-1">
              ...
              <span>•</span>
              <span>{t('accountDetails.started')} {format(new Date(account.start_date), 'MMM d, yyyy')}</span>
            </div>
          </div>
        </div>
      ...
      {account.status === 'failed' && (
        <ViolationAlert account={account} />
      )}

      {/* Phase Progress Alert */}
      {profitProgress >= 100 && account.trading_days_count >= account.min_trading_days && account.status !== 'failed' && (
        <Card className="bg-emerald-500/10 border-emerald-500/30 p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-xl font-bold text-emerald-400">{t('accountDetails.phase1Completed')}</h3>
                <p className="text-slate-300">{t('accountDetails.phase1CompletedDesc')}</p>
              </div>
            </div>
      ...
      <div className="grid lg:grid-cols-4 gap-6">
        ...
        <Card className="bg-slate-900 border-slate-800 p-6 lg:col-span-3">
          <h3 className="text-lg font-semibold text-white mb-6">{t('accountDetails.accountSummary')}</h3>
          ...
          {/* Progress Indicators */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4 mt-6 pt-6 border-t border-slate-800">
            <div>
              <div className="flex justify-between text-sm mb-2">
                <span className="text-slate-400">{t('accountDetails.profitTarget')}</span>
                <span className="text-emerald-400">{account.current_profit_percent?.toFixed(2)}% / {account.profit_target}%</span>
              </div>
              <div className="h-3 bg-slate-800 rounded-full overflow-hidden">
                <div className="h-full bg-emerald-500 rounded-full transition-all" style={{ width: `${profitProgress}%` }} />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-sm mb-2">
                <span className="text-slate-400">{t('accountDetails.dailyDrawdown')}</span>
                <span className={dailyDDUsed > 80 ? 'text-amber-400' : 'text-white'}>{account.daily_drawdown_percent?.toFixed(2)}% / {account.max_daily_dd}%</span>
              </div>
              <div className="h-3 bg-slate-800 rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all ${dailyDDUsed > 80 ? 'bg-amber-500' : 'bg-slate-600'}`} style={{ width: `${dailyDDUsed}%` }} />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-sm mb-2">
                <span className="text-slate-400">{t('accountDetails.overallDrawdown')}</span>
                <span className={overallDDUsed > 80 ? 'text-red-400' : 'text-white'}>{account.overall_drawdown_percent?.toFixed(2)}% / {account.max_overall_dd}%</span>
              </div>
              <div className="h-3 bg-slate-800 rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all ${overallDDUsed > 80 ? 'bg-red-500' : 'bg-slate-600'}`} style={{ width: `${overallDDUsed}%` }} />
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card className="bg-slate-900 border-slate-800 p-6 flex flex-col items-center justify-center">
          ...
          />
          <p className="text-slate-400 text-sm mt-4">{t('accountDetails.tradingDays')}: {account.trading_days_count}/{account.min_trading_days}</p>
        </Card>
      </div>
      ...
      {(equityMetrics.length > 0 || profitMetrics.length > 0) && (
        <div className="grid lg:grid-cols-2 gap-6">
          <EquityChart data={equityMetrics} title={t('accountDetails.equityCurve')} />
          <DailyPnLChart data={profitMetrics.length > 0 ? profitMetrics : null} />
        </div>
      )}
      ...
      <Card className="bg-slate-900 border-slate-800 p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-white">{t('accountDetails.tradeHistory')}</h3>
        ...
        <Link to={createPageUrl('ChallengeProgress')}>
          <Card className="bg-slate-900 border-slate-800 p-4 hover:border-cyan-500/50 transition-colors cursor-pointer">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-white font-medium">{t('accountDetails.challengeProgress')}</p>
                <p className="text-xs text-slate-400">{t('accountDetails.trackJourney')}</p>
              </div>
            </div>
          </Card>
        </Link>
        <Link to={createPageUrl('Analytics')}>
          <Card className="bg-slate-900 border-slate-800 p-4 hover:border-emerald-500/50 transition-colors cursor-pointer">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-white font-medium">{t('accountDetails.analytics')}</p>
                <p className="text-xs text-slate-400">{t('accountDetails.detailedStats')}</p>
              </div>
            </div>
          </Card>
        </Link>
        <Link to={createPageUrl('Rules')}>
          <Card className="bg-slate-900 border-slate-800 p-4 hover:border-purple-500/50 transition-colors cursor-pointer">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-white font-medium">{t('accountDetails.tradingRules')}</p>
                <p className="text-xs text-slate-400">{t('accountDetails.viewAllRules')}</p>
              </div>
            </div>
          </Card>
        </Link>
