...
export default function Analytics() {
  ...
  const startingBalance = equityCurveData.length > 0
    ? equityCurveData[0].balance
    : selectedAccount !== 'all'
      ? (accounts.find(acc => acc.id === selectedAccount)?.initialBalance || accounts.find(acc => acc.id === selectedAccount)?.initial_balance || 0)
      : accounts.length > 0
        ? accounts.reduce((sum, acc) => sum + (acc.initialBalance || acc.initial_balance || 0), 0)
        : 0;
    ...
    queryFn: async () => {
      try {
        if (selectedAccount === 'all') {
          ...
          const allTradesPromises = accounts.map(acc => getAccountTrades(acc.id).catch(() => []));
  ...
  const tradeColumns = [
    { header: t('analytics.symbol'), accessorKey: 'symbol', cell: (row) => <span className="font-medium text-white">{row.symbol}</span> },
    {
    ...
    { header: t('analytics.lotSize'), accessorKey: 'lot_size', cell: (row) => <span className="text-slate-300">{row.lot_size?.toFixed(2)}</span> },
    { header: t('analytics.open'), accessorKey: 'open_price', cell: (row) => <span className="text-slate-300">{row.open_price?.toFixed(2)}</span> },
    { header: t('analytics.close'), accessorKey: 'close_price', cell: (row) => <span className="text-slate-300">{row.close_price ? row.close_price.toFixed(2) : '-'}</span> },
    {
  ...
  if (isLoadingAnalytics && !analytics) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-slate-400">{t('analytics.loading') || 'Loading analytics...'}</div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <Select value={selectedAccount} onValueChange={(value) => {
          ...
          <SelectContent className="bg-slate-900 border-slate-800 text-white">
            <SelectItem value="all" className="text-white">{t('analytics.allAccounts')}</SelectItem>
            {accounts.map((acc) => {
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        ...
        <StatsCard
          ...
          gradient={statistics.totalProfit >= 0 ? "from-emerald-500 to-green-500" : "from-red-500 to-pink-500"}
        />
        <StatsCard
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-slate-900 border-slate-800 p-4">
          <p className="text-sm text-slate-400 mb-1">{t('analytics.winningTrades')}</p>
          <p className="text-xl sm:text-2xl font-bold text-emerald-400">{statistics.winningTrades}</p>
        </Card>
        <Card className="bg-slate-900 border-slate-800 p-4">
          <p className="text-sm text-slate-400 mb-1">{t('analytics.losingTrades')}</p>
          <p className="text-2xl font-bold text-red-400">{statistics.losingTrades}</p>
        </Card>
        <Card className="bg-slate-900 border-slate-800 p-4">
          <p className="text-sm text-slate-400 mb-1">{t('analytics.bestTrade')}</p>
          <p className="text-2xl font-bold text-emerald-400">+${statistics.bestTrade.toFixed(2)}</p>
        </Card>
        <Card className="bg-slate-900 border-slate-800 p-4">
          <p className="text-sm text-slate-400 mb-1">{t('analytics.worstTrade')}</p>
          <p className="text-2xl font-bold text-red-400">${statistics.worstTrade.toFixed(2)}</p>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        <Card className="bg-slate-900 border-slate-800 p-4 md:p-6">
          <h3 className="text-lg font-semibold text-white mb-4">{t('analytics.symbolDistribution')}</h3>
      ...
      {/* Trade History */}
      <Card className="bg-slate-900 border-slate-800 p-4 md:p-6">
        <h3 className="text-lg font-semibold text-white mb-4">{t('analytics.tradeHistory')}</h3>
        <DataTable
