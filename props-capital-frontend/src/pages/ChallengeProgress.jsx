...
export default function ChallengeProgress() {
  ...
  const phases = [
    ...
    },
    {
      ...
      status:
        currentPhase === "phase1"
          ? "active"
          : ["phase2", "funded"].includes(currentPhase)
            ? "completed"
            : "pending",
      ...
      requirements: [
        {
          label: t("challengeProgress.profitTarget"),
          value: `${phase1Target}%`,
          current: `${profitPercent?.toFixed(2)}%`,
          met: profitPercent >= phase1Target,
        },
        {
          label: t("challengeProgress.minTradingDays"),
          value: `${minTradingDays}`,
          current: tradingDaysCount,
          met: tradingDaysCount >= minTradingDays,
        },
        {
          label: t("challengeProgress.maxDailyDD"),
          value: `${maxDailyDD}%`,
          current: `${dailyDrawdownPercent?.toFixed(2)}%`,
          met: dailyDrawdownPercent <= maxDailyDD,
        },
        {
          label: t("challengeProgress.maxOverallDD"),
          value: `${maxOverallDD}%`,
          current: `${overallDrawdownPercent?.toFixed(2)}%`,
          met: overallDrawdownPercent <= maxOverallDD,
        },
      ],
    },
    {
          ...
          ? [
              {
                ...
                value: `${phase2Target}%`,
                current: `${profitPercent?.toFixed(2)}%`,
                met: profitPercent >= phase2Target,
              },
              {
                label: t("challengeProgress.minTradingDays"),
                value: `${minTradingDays}`,
  ...
  const getStatusBorder = (status) => {
    switch (status) {
      ...
      case "active":
        return "border-cyan-500 ring-4 ring-cyan-500/20";
      case "locked":
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="flex items-center gap-4">
            <ProgressRing
              progress={Math.max(
                0,
                Math.min((profitPercent / phase1Target) * 100, 100),
              )}
              value={profitPercent}
              size={80}
              strokeWidth={6}
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <h2 className="text-sm sm:text-xl font-bold text-white">
                  ...
                  {t("challengeProgress.challenge")}
                </h2>
                <StatusBadge status={currentPhase} />
              </div>
              <p className="text-slate-400">
                {t("challengeProgress.account")} #{accountNumber} • {platform}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-6">
            ...
            <div className="text-center">
              <p className="text-base sm:text-2xl font-bold text-amber-400">
                {dailyDrawdownPercent?.toFixed(1)}%
              </p>
              <p className="text-sm text-slate-400">
                {t("challengeProgress.dailyDDUsed")}
              </p>
            </div>
          </div>
        </div>
      ...
      <Card className="bg-slate-900 border-slate-800 p-6">
        <h3 className="text-lg font-semibold text-white mb-6">
          {t("challengeProgress.yourJourney")}
        </h3>
        ...
        <div className="relative mb-8">
          ...
          <div className="flex justify-between mt-2">
            {phases.map((phase, index) => (
              ...
              >
                <div
                  className={`w-4 h-4 rounded-full ${getStatusColor(phase.status)} -mt-3 border-2 border-slate-900`}
                />
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 md:gap-4">
          {phases.map((phase, index) => (
            <div
              key={phase.id}
              className={`relative rounded-xl border-2 p-2 sm:p-4 transition-all ${getStatusBorder(phase.status)} ${
                phase.status === "active"
                  ? "bg-cyan-500/5"
            ...
            >
              ...
              >
                {phase.status === "completed" ? (
                  ...
                  <phase.icon
                    className={`w-6 h-6 ${phase.status === "active" ? "text-cyan-400" : "text-slate-400"}`}
                  />
                )}
              </div>
              ...
              <h4
                className={`font-semibold mb-1 ${
                  phase.status === "locked" ? "text-slate-500" : "text-white"
                }`}
              >
                {phase.name}
              </h4>
              ...
              ?.requirements.map((req, i) => (
                ...
                >
                  ...
                  <div className="flex items-baseline justify-between">
                    <span
                      className={`text-xl font-bold ${req.met ? "text-emerald-400" : "text-white"}`}
                    >
                      {req.current}
                    </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
        ...
        <Card className="bg-gradient-to-br from-purple-500/10 to-purple-500/5 border-purple-500/30 p-6">
          ...
          <p className="text-sm text-slate-400 mt-1">
            {t("challengeProgress.processedWithin24h")}
          </p>
        </Card>
      ...
      <Card className="bg-gradient-to-r from-cyan-500/10 to-purple-500/10 border-cyan-500/30 p-6 text-center">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
          ...
          <Link to={createPageUrl("Rules")} className="w-full sm:w-auto">
            <Button
              variant="outline"
              className="border-slate-700 bg-slate-800/50 text-white  w-full"
            >
