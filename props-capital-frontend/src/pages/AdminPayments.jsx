...
import { adminGetAllPayments, adminGetPaymentStatistics, adminRefundPayment } from '@/api/admin';
...
export default function AdminPayments() {
  ...
  const filteredPayments = displayPayments.filter(payment => {
    const matchesSearch = payment.trader_id?.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const columns = [
    ...
    },
    {
      ...
      accessorKey: 'amount',
      cell: (row) => <span className="text-emerald-400 font-bold">${row.amount}</span>
    },
    {
      ...
      cell: (row) => {
        ...
        const displayMethod = translatedMethod === translationKey
    ...
    { header: t('admin.payments.table.status'), accessorKey: 'status', cell: (row) => <StatusBadge status={row.status} /> },
    {
      header: t('admin.payments.table.date'), accessorKey: 'created_date', cell: (row) => {
        try {
      ...
      cell: (row) => (
        <div className="flex items-center gap-2">
          {row.status === 'completed' && (
            <Dialog open={refundDialogOpen && selectedPayment?.id === row.id} onOpenChange={(open) => {
              ...
              <DialogContent className="bg-slate-900 border-slate-800 w-[95vw] sm:w-full sm:max-w-md p-4 sm:p-6">
                <DialogHeader>
                  <DialogTitle className="text-white text-base sm:text-lg md:text-xl">{t('admin.payments.refundTitle') || 'Refund Payment'}</DialogTitle>
                </DialogHeader>
                <div className="space-y-3 sm:space-y-4 mt-3 sm:mt-4">
                  <div>
                    <p className="text-slate-400 text-xs sm:text-sm">
                      {t('admin.payments.refundConfirm') || `Are you sure you want to refund $${row.amount} to ${row.trader_id}?`}
                    </p>
                  </div>
                  <div className="space-y-1.5 sm:space-y-2">
                    <Label className="text-slate-300 text-xs sm:text-sm">{t('admin.payments.refundReason') || 'Refund Reason (Optional)'}</Label>
                    <Textarea
                      value={refundReason}
                      onChange={(e) => setRefundReason(e.target.value)}
                      placeholder={t('admin.payments.refundReasonPlaceholder') || 'Enter reason for refund...'}
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 justify-end mt-4">
                    <Button
                      ...
                      className="border-slate-600 text-white w-full sm:w-auto order-2 sm:order-1"
                    >
                    ...
                    >
                      {refundMutation.isPending ? (t('admin.payments.refunding') || 'Processing...') : (t('admin.payments.confirmRefund') || 'Confirm Refund')}
                    </Button>
                  </div>
                </div>
              </DialogContent>
          ...
          {row.status === 'refunded' && (
            <div className="flex flex-col">
              <span className="text-xs text-slate-400">{t('admin.payments.actions.refunded')}</span>
              {row.refund_reason && (
                <span className="text-xs text-slate-500 mt-1">{row.refund_reason}</span>
              )}
            </div>
          )}
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-white">{t('admin.payments.title')}</h1>
          <p className="text-sm sm:text-base text-slate-400">{t('admin.payments.subtitle')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatsCard
          ...
          change={statistics.revenueChangePercent !== undefined ? t('admin.payments.stats.changeThisMonth', { percent: Math.round(statistics.revenueChangePercent) }) : undefined}
          changeType={statistics.revenueChangePercent >= 0 ? 'positive' : 'negative'}
        />
        <StatsCard
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent className="bg-slate-800 border-slate-700 text-white">
              <SelectItem value="all" className="text-white">{t('admin.payments.filter.allStatus')}</SelectItem>
              <SelectItem value="completed" className="text-white">{t('admin.payments.filter.completed')}</SelectItem>
              <SelectItem value="pending" className="text-white">{t('admin.payments.filter.pending')}</SelectItem>
              <SelectItem value="failed" className="text-white">{t('admin.payments.filter.failed')}</SelectItem>
              <SelectItem value="refunded" className="text-white">{t('admin.payments.filter.refunded')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </Card>
