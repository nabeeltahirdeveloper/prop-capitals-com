...
import { getUserPayouts, requestPayout, getPayoutStatistics } from '@/api/payouts';
...
export default function TraderPayouts() {
  ...
  const accounts = allAccounts.filter(account =>
    account.status === 'ACTIVE' && account.phase === 'FUNDED'
  );
  ...
  const createPayoutMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setIsOpen(false);
      setRequestForm({ tradingAccountId: '' });
    },
  });
  ...
  const displayPayouts = (payouts || []).map(payout => ({
    ...
    processed_date: payout.status === 'PAID' || payout.status === 'paid'
      ? (payout.updatedAt || payout.processed_date || payout.processedAt)
      : null,
  ...
  const payoutColumns = [
    {
      ...
      cell: (row) => row.created_date ? format(new Date(row.created_date), 'MMM d, yyyy') : '-'
    },
    {
      header: t('payouts.amount'),
      accessorKey: 'amount',
      cell: (row) => <span className="text-white font-semibold">${row.amount?.toLocaleString()}</span>
    },
    {
      ...
      cell: (row) => {
        ...
        return <span className="capitalize">{methodMap[row.payment_method] || row.payment_method?.replace(/_/g, ' ')}</span>;
      }
    },
    {
    ...
    },
    {
      ...
      cell: (row) => row.processed_date ? format(new Date(row.processed_date), 'MMM d, yyyy') : '-'
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">{t('payouts.title')}</h1>
        ...
        >
          <SelectTrigger className="w-[280px] bg-slate-900 border-slate-800 text-white">
            <SelectValue placeholder={t('payouts.selectAccount') || 'Select Account'} />
          </SelectTrigger>
          <SelectContent className="bg-slate-900 border-slate-800 text-white">
            <SelectItem value="all" className="text-white">{t('payouts.allAccounts') || 'All Accounts'}</SelectItem>
            {accounts.map((acc) => {
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent className="bg-slate-900 border-slate-800">
            <DialogHeader>
              <DialogTitle className="text-white">{t('payouts.requestPayout')}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 mt-4">
              <div className="space-y-2">
                <Label className="text-slate-300">{t('payouts.selectAccount')}</Label>
                <Select onValueChange={(value) => setRequestForm({ ...requestForm, tradingAccountId: value })}>
                  <SelectTrigger className="bg-slate-800 border-slate-700 text-white">
                    <SelectValue placeholder={t('payouts.selectFundedAccount')} />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                <Label className="text-slate-300">{t('payouts.amount')}</Label>
                <Input
                  type="number"
                  placeholder={t('payouts.enterAmount')}
                  value={requestForm.amount}
                  onChange={(e) => setRequestForm({ ...requestForm, amount: e.target.value })}
                  className="bg-slate-800 border-slate-700 text-white placeholder:text-slate-500"
                />
                <p className="text-xs text-slate-500">{t('payouts.minimum')}: ${settings.minimumAmount || 50}</p>
              </div>

              <div className="space-y-2">
                <Label className="text-slate-300">{t('payouts.paymentMethod')}</Label>
                <Select onValueChange={(value) => setRequestForm({ ...requestForm, payment_method: value })}>
                  <SelectTrigger className="bg-slate-800 border-slate-700 text-white">
                    <SelectValue placeholder={t('payouts.selectPaymentMethod')} />
                  </SelectTrigger>
                  <SelectContent className="bg-slate-800 border-slate-700 text-white">
                    {(settings.availablePaymentMethods && settings.availablePaymentMethods.length > 0
                      ...
                      : [
                        { id: 'bank_transfer', name: t('payouts.bankTransfer') },
              ...
              <div className="space-y-2">
                <Label className="text-slate-300">{t('payouts.paymentDetails')}</Label>
                <Textarea
                  placeholder={t('payouts.paymentDetailsPlaceholder')}
                  value={requestForm.payment_details}
                  onChange={(e) => setRequestForm({ ...requestForm, payment_details: e.target.value })}
                  className="bg-slate-800 border-slate-700 text-white placeholder:text-slate-500"
                />
              </div>
              ...
              <Button
                ...
                disabled={createPayoutMutation.isPending || !requestForm.tradingAccountId}
              >
                {createPayoutMutation.isPending ? t('payouts.submitting') : t('payouts.submitRequest')}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
      ...
      <Card className="bg-slate-900 border-slate-800 p-6">
        <h3 className="text-lg font-semibold text-white mb-4">{t('payouts.payoutSchedule')}</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {[
            { icon: Calendar, title: t('payouts.payoutFrequency'), value: translateFrequency(settings.frequency) },
            { icon: Clock, title: t('payouts.processingTime'), value: translateProcessingTime(settings.processingTime) },
            { icon: DollarSign, title: t('payouts.minimumAmount'), value: `$${settings.minimumAmount || 50}` },
            { icon: CheckCircle, title: t('payouts.payoutFee'), value: translateFees(settings.fees) },
          ].map((item, i) => (
      ...
      {/* Payment Methods */}
      <Card className="bg-slate-900 border-slate-800 p-6">
        <h3 className="text-lg font-semibold text-white mb-4">{t('payouts.availablePaymentMethods')}</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          {(settings.availablePaymentMethods && settings.availablePaymentMethods.length > 0
            ? settings.availablePaymentMethods.map(method => ({
            ...
            : [
              { icon: CreditCard, name: t('payouts.bankTransfer'), time: t('payouts.bankTransferTime') },
              { icon: Bitcoin, name: t('payouts.cryptocurrency'), time: t('payouts.cryptoTime') },
              { icon: CreditCard, name: t('payouts.paypal'), time: t('payouts.paypalTime') },
              { icon: CreditCard, name: t('payouts.wise'), time: t('payouts.wiseTime') },
            ]
          ).map((method, i) => (
      ...
      <Card className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border-purple-500/30 p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-lg font-semibold text-white">{t('payouts.yourProfitSplit')}</h3>
              <p className="text-slate-400">{t('payouts.profitSplitDesc')}</p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-4 sm:gap-6">
            <div className="text-center">
              <p className="text-2xl sm:text-3xl font-bold text-purple-400">{settings.profitSplit?.base || 80}%</p>
              <p className="text-xs text-slate-400">{t('payouts.currentSplit')}</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-bold text-emerald-400">{settings.profitSplit?.withScaling || 90}%</p>
              <p className="text-xs text-slate-400">{t('payouts.withScaling')}</p>
            </div>
      ...
      {/* Payout History */}
      <Card className="bg-slate-900 border-slate-800 p-6">
        <h3 className="text-lg font-semibold text-white mb-4">{t('payouts.payoutHistory')}</h3>
        <DataTable
      ...
      <Card className="bg-slate-900 border-slate-800 p-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-white font-semibold mb-1">{t('payouts.needHelp')}</h3>
