  ...
  if (typeof t === "number") {
    ...
    if (normalized < minTime || normalized > maxTime) {
      if (import.meta.env?.DEV) {
        console.warn(`⚠️ Suspicious timestamp rejected: ${normalized} (${new Date(normalized * 1000).toISOString()})`);
      }
 ...
 */
export function isWithinTradingSession(timeSeconds, sessionTZ = 'America/New_York') {
  ...
  const sessionDate = new Date(date.toLocaleString('en-US', { timeZone: sessionTZ }));
