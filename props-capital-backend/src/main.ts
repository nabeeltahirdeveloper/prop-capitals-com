...
async function bootstrap() {
  ...
  // NestJS's gateway scanner (SocketModule) creates and OWNS the single
  // engine.io Server through this adapter, and wires every @WebSocketGateway
  // — root candles (/), /trading and /support — including each afterInit,
  // @SubscribeMessage handler and handleConnection, exactly once.
  //
  // Do NOT also manually `new Server(app.getHttpServer())` here: a second
  // engine.io attach re-registers an 'upgrade' listener on the same HTTP
  // server, which crashes the process with "server.handleUpgrade() was called
  // more than once with the same socket" the moment any client upgrades to the
  // websocket transport. (The scanner always runs here — the gateways import
  // `@WebSocketGateway` from @nestjs/websockets, so if that package were
  // unresolvable the app would not boot at all. Confirm on startup via each
  // gateway's "... initialized" / "Candle emitter started" log line.)
  app.useWebSocketAdapter(new SocketIoAdapter(app));
