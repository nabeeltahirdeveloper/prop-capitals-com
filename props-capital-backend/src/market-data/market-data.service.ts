...
export class MarketDataService {
   ...
   */
  async getHistory(symbol: string, timeframe: string = 'M5', limit: number = 100): Promise<Candlestick[]> {
    try {
      if (this.isForexSymbol(symbol)) {
        // ✅ Twelve Data Real History
        return await this.twelveDataService.getHistory(symbol, timeframe, limit);
      } 
  ...
  async getUnifiedPrices(symbolList: string[] = []) {
    const targetSymbols = symbolList.length > 0 
      ? symbolList 
