...
export class AdminPayoutsService {
  ...
  async updateStatus(id: string, status: string, reason?: string) {
    ...
    if (payout.userId) {
      const amount = Number(payout.amount).toLocaleString('en-US', {
      ...
      if (status === PayoutStatus.APPROVED) {
        ...
        await this.notificationsService.create(
          payout.userId,
          'Payout Rejected',
          `Your payout request of $${amount} has been rejected.${reasonText} Please contact support for more information.`,
