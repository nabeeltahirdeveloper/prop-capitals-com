...
export class PendingOrderRegistryService implements OnModuleInit {
  ...
  private getBucket(
    ...
    if (order.type === 'BUY' && order.orderType === 'LIMIT')
      return book.buyLimit;
    if (order.type === 'SELL' && order.orderType === 'LIMIT')
      return book.sellLimit;
    if (order.type === 'BUY' && order.orderType === 'STOP') return book.buyStop;
    if (order.type === 'SELL' && order.orderType === 'STOP')
      return book.sellStop;
    if (order.type === 'BUY' && order.orderType === 'STOP_LIMIT')
      return book.buyStopLimit;
    if (order.type === 'SELL' && order.orderType === 'STOP_LIMIT')
      return book.sellStopLimit;

    throw new Error(
      `Unsupported order combination: ${order.type} ${order.orderType}`,
    );
  }
