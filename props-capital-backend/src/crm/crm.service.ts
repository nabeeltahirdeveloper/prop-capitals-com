...
export class CrmService {
    ...
    async getAllLeads(filters?: {
        ...
        if (filters?.fromDate || filters?.toDate) {
            ...
            if (filters?.fromDate) {
                ...
                if (typeof filters.fromDate === 'string' && filters.fromDate.length <= 10) {
                    d.setHours(0, 0, 0, 0);
                }
    ...
    async updateLead(id: string, data: {
        ...
        if (data.leadStatus === LeadStatus.CONVERTED && lead.leadStatus !== LeadStatus.CONVERTED) {
            if (!data.convertedAt) {
    ...
    async logActivity(leadId: string, activityType: LeadActivityType, notes?: string) {
    ...
    async importLeadsFromCSV(csvData: string) {
            ...
            headers.forEach((header, index) => {
                ...
                if (value) {
                    ...
                    if (fieldName === 'leadStatus') {
                        ...
                        leadData.onlineStatus = value.toUpperCase() === 'ONLINE' ? LeadOnlineStatus.ONLINE : LeadOnlineStatus.OFFLINE;
                    ...
                    } else if (fieldName === 'leadReceivedDate' || fieldName === 'convertedAt') {
        ...
        for (const leadData of leads) {
            try {
                ...
                console.error(`Error creating lead ${leadData.email as string}:`, error);
            }
        }
    ...
    async getLeadStats() {
        const [all, newLeads, contacted, qualified, callback, followUp, converted, lost] = await Promise.all([
    ...
    async getFtdStats(filters?: {
        ...
        const totalDeposits = ftdLeads.reduce((sum, lead) => sum + (lead.ftdAmount || 0), 0);
    ...
    async getMeetingStats(filters?: any) {
        ...
        return {
            ...
            meetings: allMeetings.filter(m => m.type?.toLowerCase() === 'meeting').length,
        };
    }
