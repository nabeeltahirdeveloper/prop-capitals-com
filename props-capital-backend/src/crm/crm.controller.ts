...
export class CrmController {
  ...
  // ─── API Key Management (must be above :id routes) ───

  @Post('api-keys')
  async generateApiKey(@Body() body: { name: string }) {
    return this.crmService.generateApiKey(body.name);
  }

  @Get('api-keys')
  async listApiKeys() {
    return this.crmService.listApiKeys();
  }

  @Delete('api-keys/:id')
  async revokeApiKey(@Param('id') id: string) {
    return this.crmService.revokeApiKey(id);
  }
